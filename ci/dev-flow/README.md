...
| 2 建置 | `system.md`「Constraint」的建置指令 | 擋 |
...
4. Settings → Branches → 主線的 branch protection 勾三項：
   ...
   - **Require review from Code Owners**：動到 `system.md`、`requirements/`、`modules.md` 的 PR 要架構負責人 approve
...
**只改立案的 PR（`plan/<slug>`：需求、里程碑、全域 Law）會紅嗎？** 不會。它只動 `.design/`；里程碑還沒有切片是警訊不是紅。
