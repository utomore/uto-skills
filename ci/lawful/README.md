...
2. `ref` 釘在 uto-skills 的一個 tag(範本寫的是目前的 tag,要升級就換);`ghc-version` 與 `cabal-version` 照專案的 `cabal.project`。
...
2. `UTO_SKILLS_REF` 釘在 uto-skills 的一個 tag(範本寫的是目前的 tag,要升級就換;分支與 commit SHA 也能填);`image` 的 GHC 版本照專案。
