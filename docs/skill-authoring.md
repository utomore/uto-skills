...
各 skill 的 `description` 是**永遠在 context** 的(約 4.5 KB / 10 個 skill),但那是觸發判斷的必要成本,不是優化對象。
...
**拆片最大的風險是漏讀**,唯一的防線是:每個 `SKILL.md` 開頭必須**明列讀哪幾片、以及為什麼要讀**,不能只寫「遵守共用慣例」。加分片或改分片條件時,十個 `SKILL.md` 的開頭都要同步檢查。
...
- **不要把單一 skill 的主流程拆成多個檔案。** `_shared/` 拆片是因為它被十個 skill 共用、需求差異大;單一 skill 的流程拆開只會提高漏讀率,省下的也有限。
