...
const BLOCKS = {
  ...
  'global-laws': { none: ['branch', 'system', 'requirements', 'modules', 'gaps', 'lintglobal', 'status'], top: ['branch', 'system', 'requirements', 'modules', 'gaps', 'lintglobal', 'status'], milestone: ['branch', 'system', 'requirement', 'modules', 'journal', 'bound', 'gaps', 'lintglobal', 'status'] },
...
export function briefCommand(root, design, source, adapter, skill, target, { fingerprint = false, noRules = false, statusText = null, part = 0, of = 0 } = {}) {
  ...
  const block = {
    ...
    // 里程碑:它那條需求的需求檔全文;文檔:綁它的里程碑,與那幾條需求的需求檔
    requirement: () => {
      if (ms) return out.push(...requirementLines(ms.q));
      const rows = [];
      const reqs = new Set();
      // 靠修訂這一份達成的里程碑:它的修訂記錄裡要有一條 REV 的依欄寫了那條里程碑的全名
      ...
      const revised = (q, m) => {
        ...
        return d && d.byRevision ? `;靠修訂這一份達成,${d.cited ? 'REV 的依欄已經引用它' : `還沒有 REV 的依欄引用它:REV 的依欄寫 ${m.fullName}`}` : '';
      };
      ...
      out.push('# 這份文檔朝向哪裡', '', ...(rows.length ? rows : ['(沒有任何里程碑綁它)']), '');
