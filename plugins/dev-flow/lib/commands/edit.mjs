...
export function invariantAdd(design, title, { kind = 'invariant' } = {}) {
  ...
  fs.writeFileSync(file, lines.join('\n'));
  return { text: [`${id} 寫進 .design/system.md「全域 Law」的領域不變量`, `還沒有三行式:從切片抽上去的,把出處那條 law 的三行照搬過來(識別字只用最內層的匯出與型別名);最內層的型別還沒出現的先留一句話。有了三行,build 派 qa 寫歸屬 "${id}#LAW" 的測試`].join('\n'), exitCode: 0, id };
}
