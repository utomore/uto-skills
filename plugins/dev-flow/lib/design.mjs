...
// 名詞：.design/vocabulary.md，標題「名詞」底下一張表（名詞 | 定義 | 型別）；領域名詞只在那裡定義。
// 專案根目錄的 CLAUDE.md 有一行裸的 `@.design/vocabulary.md` 把它匯入，每一場 session 開場就載入（寫在反引號裡不算匯入）。
...
export const VOCABULARY_IMPORT = '@.design/vocabulary.md';
export function readGlossary(root) {
  const claudeText = read(path.join(root, 'CLAUDE.md'));
  const vocabFile = path.join(root, '.design', 'vocabulary.md');
...
export function readDesign(root) {
  ...
  return {
    ...
    abstractsDir,
    system,
    // 名詞表住 .design/vocabulary.md，專案根目錄的 CLAUDE.md 匯入它
