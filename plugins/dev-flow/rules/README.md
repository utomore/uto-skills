...
| 名詞 | 在哪 |
|---|---|
| 願景、Constraint（硬性限制與工具要讀的那幾行） | features.md「system.md」 |
| 專案的名詞表（`.design/vocabulary.md`，專案根目錄的 `CLAUDE.md` 匯入它）、型別欄、誰寫 | features.md「名詞」 |
| 需求、`R-n`、驗收、優先與它的四級、里程碑、`M-n`、全名 `M-n-<slug>`、綁定、還沒有切片、需求的依賴 | features.md「願景、需求與里程碑」 |
...
| 決策紀錄、`journal/<鍵>.md`、切片六節、Verification、合併時要看、開發者推翻了某一列、收尾時對回最後的程式碼 | roles.md「決策紀錄」 |
