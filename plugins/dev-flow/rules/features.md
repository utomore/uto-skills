...
文檔只寫程式碼裝不下的東西:測試存在之前的 laws、為什麼這樣決定、跨過對外邊界的資料流。型別、簽名、匯出住程式碼;文檔引用,工具對帳。它們在設計階段就住進去:一份文檔拍板 `ready` 之前,它的型別與每條簽名的骨架已經在同一條設計分支的程式碼裡,兩者一起合進主線,建構才開始(roles.md「骨架與基線」「分支與所有權」)。
...
frontmatter:`language`(選 adapter;前後端各一種語言的專案寫 `[<目錄> = <adapter>, <目錄> = <adapter>]`,tooling.md「language adapter」)、`updated`。六節:
