...
線上壞了、使用者回報了一件事，由 `dev-flow:incident` 接。它不寫測試、不寫實作、不改 `.design/` 的任何檔，做三件事，做完交給一個 skill:

1. **重現**：把現象寫成一組具體的輸入、實際的輸出與預期的輸出（請求、參數、回應、日誌那一行；預期是開發者或使用者說的，不自己推）。在**使用者手上的那一版**上跑出同一個結果：那條需求上線在哪個發布的 tag 由 `devflow release` 讀（tooling.md「發布」），在那個 tag 開一棵暫時的工作樹重現；還沒上線的需求在主線上重現。再在主線上跑一次：主線上已經不壞，回報「主線已修好，等下一次發布」與修好它的 commit，不交棒。重現用的腳本放暫存區，不進 repo、不標歸屬，跑完連同工作樹一起收掉。重現不出來就停下，回報試了什麼、缺什麼（版本、資料、設定），不猜原因、不改程式碼試試看。
2. **歸因**：從「全域 Law」區的對外 I/O 表找這個輸入從哪一端進來、進哪份 feature 的 `!` 列，沿 Steps 找結果第一次變錯的那一個 step，再對它的 law:
   ...
   | 不在程式碼裡（設定、基礎設施、第三方服務） | 不歸 dev-flow：回報開發者，不交棒 |

3. **交棒**：寫好一行指令，參數是全名加上來源——事故的原句、重現的那一組輸入與實際、預期的輸出、對到的 law（沒有寫「無」）——自動接上那個 skill；它的 REV 依欄寫這一段。

- **會紅的測試只活在 build 分支上**：接手的 skill 把重現的那一組寫進文檔，`dev-flow:build` 的 qa 從文檔寫出測試，首跑在現有的程式碼上該紅——紅了才證明這條測試抓得到這次事故；refactor 把它調綠，整套綠才整合。主線上的測試一律是綠的，事故的紅不進主線。
- 修好的那一版要等下一個發布的 tag 才到使用者手上：`devflow release` 會把那條需求列回未上線，直到打了新的 tag。簽過驗收的需求，證據因為這次修訂翻掉的，`devflow status` 列成待重審，修好之後開發者再簽一次（features.md「需求的達成只有人判得了」）。
