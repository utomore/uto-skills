...
| **整合** | Integration MUST NOT reduce Law satisfaction, and MUST NOT change a Law: it proposes, the developer approves, law-design or glaws-revise writes.(合併之前成立的每一條 law,合併之後都要仍然成立;整合不改任何一條 law,只提建議,開發者批准,scope law 由 law-design、全域 Law 由 glaws-revise 落筆) | `dev-flow:integrate` | 整合分支 | 幾條達成的分支合成一條、整套綠、仲裁、ADR、PR |
...
既有文檔的改動是另一條路:`dev-flow:law-design` 的第二種情形(既有文檔要改;文檔先行,features.md「修訂(REV)」)與 `dev-flow:abstract`(收整)各自開 `build/<全名>`,改完條文同樣自動接上 build。全域 Law 的變更走 `dev-flow:glaws-revise`(laws.md「全域 Law 的變更」)。
...
| `build/<文檔全名>` | `dev-flow:law-design`(既有文檔要改、文檔已在主線上)、`dev-flow:abstract` | 被修訂的文檔,或新的 abstract |
...
- **宣告歸設計這一側,本體歸實作這一側**:Steps 上的簽名與它們用到的型別,切片定案後只有 `dev-flow:law-design`、`dev-flow:abstract` 能改;refactor 只動本體、私有 helper 與型別的內部表示。qa 與 refactor 非動宣告不可就是 GAP。
...
- 不動:需求檔的其餘部分(一句話、驗收那一句、優先、里程碑與調整的列)、全域 Law 區(領域不變量、層表);別份已經在主線上的文檔與它的 step 本體、別人的測試檔。唯一的例外:需求的驗收或一條領域不變量還只有一句話、而這條切片剛好讓它引用得到的簽名出現了,`dev-flow:law-design` 與開發者把那一句話寫成三行;那一句話本身不改。切片非動到別份文檔的簽名不可,那是那份文檔的修訂:在同一棵工作樹上走 `dev-flow:law-design`(既有文檔要改的那一種情形),REV 的連動欄寫明。
...
- **未實作標記**:修訂新增的 step,`dev-flow:law-design` 先在程式碼裡宣告它,本體是 adapter 的標記,訊息帶 `F-00x#name`,首跑的紅燈才看得出打到哪個 step。**不得回傳假值**(回 `0`、`[]`、`null` 會讓測試假綠)。
...
| 時機 | 誰派、在哪 |
|---|---|
| 一份文檔的 build 本波全綠後,`devflow status` 顯示它讓某條需求的里程碑全部達成,而那條需求的驗收有三行式卻沒有測試;或 law-design 在這條分支上把一條領域不變量寫成了三行 | 同一條 build 分支上再派一次 qa,目標是那一條;綠了才收尾 |
...
- 這種測試紅的歸因不是某個 step 的實作:里程碑全部達成而驗收測試沒過,代表里程碑切漏了、或驗收寫錯;領域不變量紅,代表某一條切片違反了它。conductor 開 GAP(角色 conductor,目標寫 `R-n#ACCEPT` 或 `INV-n#LAW`)停下,需求的回 `dev-flow:require-design`,領域不變量的照仲裁歸因到違反它的那份文檔、或回 `dev-flow:glaws-revise`;不讓 qa 放寬斷言。
...
- open 的 GAP 清單,各附「需要回答什麼」;回答走 `dev-flow:law-design`(既有文檔要改的那一種情形),結案的 step 下一波重派。
...
開發者的決定只在四個地方發生:立案與需求的對談(`dev-flow:kickoff` / `require-design`)、Law 對談(`dev-flow:law-design` / `abstract`)、回答 GAP 與選定 law 的調整(scope law 在 `dev-flow:law-design`、全域 Law 在 `dev-flow:glaws-revise`,先看影響範圍再選選項)、整合的仲裁(`dev-flow:integrate`)。開發者只說,文檔一律由 skill 寫。build 不替開發者做契約級決定,也不事後追認。
...
| Goal / Scope | 這份紀錄服務的需求與里程碑那一句、這一片做到了什麼、屬於這條里程碑卻明確沒做的 | law-design |
...
- **仲裁**,一次一條,問開發者:
  ...
  - 三個選項,各附當下成本、之後的代價、可不可逆。它們是給開發者的**變更建議**:整合者不准自己選,也不准因為哪一個讓合併最快過就推它:
    ...
    | 收窄定義域 | 兩條 law 各自的 `forall` / `given` 排除對方的情境(金額不為負,排除退款單)。兩份文檔各一次 `dev-flow:law-design` 的修訂,只重派 qa 改那條測試;實作多半不必動 |
    | 提煉上層 Law | 建議立一條領域不變量(要過准入四條);開發者明確批准後,由 `dev-flow:glaws-revise` 走「全域 Law 的變更」落筆(`devflow invariant add`),不是整合者。兩條分支都退回,各自 `dev-flow:law-design` 讓自己的 law 服從它,再 build |
  - 開發者選了什麼,寫成 GAP(角色 conductor,目標那條 law,「需要回答什麼」寫選項與開發者的原話)放進被退回的那條分支的 `gaps.md` 並 commit;`dev-flow:law-design` 的 REV 依欄引用它。整合者自己不改 law、不現場合併兩邊的邏輯。
- **全域 Law 的變更建議**:合併後紅的是全域 Law(`lint global` 的紅、領域不變量的測試紅),或決策紀錄的 Constraint 欄顯示某一條全域 Law 逼出了沒道理的決定,整合者照同一個格式提建議:反例、選項(一定含「不改,退回違反它的那條分支」)、各自的代價。**任何全域 Law 的修改、放寬、替換或刪除,都必須經開發者明確批准;整合者只能提出變更建議,不得自行決定變更,也不直接修改全域 Law。** 批准的那個選項寫成 GAP(角色 conductor,目標寫那條全域 Law),由 `dev-flow:glaws-revise` 完成,完成後重新驗證受影響的工作(laws.md「全域 Law 的變更」);這次整合只合不受它影響的分支。
