...
| 子命令 | 做什麼 |
|---|---|
| `status [--tests <log> \| --run]` | 派工報告。laws 綠幾條、需求與目標的 Law 成立與否要有測試輸出:`--tests` 給留檔的輸出,`--run` 在專案根目錄跑 `system.md` 的整套指令;兩者都沒給、或輸出裡一條 `F-00x#LAW-n` 標記都沒有,就列「未跑」並在開頭寫明。多語言專案每側一份輸出:`--tests <目錄>=<log>,<目錄>=<log>`,目錄是 `language` 欄宣告的,指到沒宣告的目錄就不讀;只給一份輸出就每一側的 adapter 都掃它一遍;`--run` 照整套指令每側各跑一道、各用自己的 adapter 掃。專案根目錄有 `.git` 時查分支,有 `build/<全名>` 分支而且它還沒合進主線的線列成建構中 |
...
| `migrate objectives [--write]` | 目標還擠在一份 `objectives.md` 的樹,換成 `system.md`「需求」與 `objectives/` 體系:先印帳本,`--write` 才落地。每個目標生一條需求(一句話照抄、判準當 Law)寫進「需求」節並讓目標繼承它、每個目標拆成 `objectives/R-x-O-y-<slug>.md`(slug 從第一條綁定的 feature 推)、「目的」併成「願景」第二段、優先各級那行搬進「語言與工具」,最後刪 `objectives.md` |
| `migrate <.design>` | 盤點 `subsystems/` 體系的 `.design`,印一份帳本,不改任何檔:每份任務文檔的介面在程式碼裡對到幾條、四格 law 翻成三行草稿、共用簽名列成 abstract 候選、退場清單、人要判的清單 |

exit code:`status` 盤點 = 驗收(有未達成的文檔、open GAP、或需求 Law 未成立即 1),`status --doc` / `--module` = 查得到 0、查不到 1;`lint` 一律 0 / 1。

## status 報告

給開發者讀的派工報告,版面固定。第一行印願景的第一段(還是模板就不印,列警訊),第二行是數字(需求與 Law 成立數,成立的再分測試幾條、推得幾條;目標、里程碑、調整、feature、abstract、文檔、待實作 step、open GAP);接著三張表:

- **需求**:每條需求一列(一句話、Law 成立 / 未成立 / 未知與判定來源、底下的目標、目標 Law 成立幾個、里程碑達成幾條、調整達成幾條),照它最高優先的目標排。**這一段答的是「需求成立了沒」**
- **目標**:先一行 `system.md`「語言與工具」宣告的優先各級,再每個目標一列(需求、優先、一句話、Law 成立與否與來源、里程碑總數、里程碑達成、完成度、調整達成),照優先排;每個沒達成的目標一行「下一個里程碑」,附綁定的文檔各在什麼狀態,還沒綁的寫成待 claim;建置路線達成的目標改寫下一個沒達成的調整與它的狀態;最後一行列沒有被任何里程碑綁定的 feature。**這一段答的是「我們有沒有朝向目標」**
- **文檔**:每份文檔一列

然後八段:

1. 今天能開幾條線:`ready`、沒 open GAP、引用的 abstract 全部達成的文檔;每條附它綁在哪個目標與里程碑,照目標優先排。專案根目錄有 `.git` 時查分支:有 `build/<全名>` 分支而且它還沒合進主線的列成建構中,不算能開;已經合進主線的分支是殘留,不擋任何線。兩條能開的線的 step 住同一個檔案,附一行提示:同時開,整合時那個檔案兩邊都動
...
分母是 `system.md`「Features」表的份數與「需求」的條數,以及 `objectives/` 的檔數與各檔的里程碑與調整數。

## 測試歸屬

每條 law 與 example 至少一條測試,測試宣告歸屬 `F-00x#LAW-n`;需求與目標寫了三行式的 Law 各一條驗收測試,歸屬 `R-n#LAW` / `O-n#LAW`(一條需求或目標只有一條 Law,沒有序號)。歸屬有兩種寫法,同一個意思:

| 寫法 | 用在 | 例 |
|---|---|---|
| 字串 `F-001#LAW-1`、`R-1#LAW` | 測試名可以是任意字串 | `describe("F-001#LAW-1 …")`、`t.Run("R-1#LAW", …)` |
| 識別字 `f_001__law_1`、`r_1__law` | 測試名必須是識別字(`-` 換 `_`,`#` 換 `__`,大小寫不拘) | `def test_f_001__law_1_rotate(…)`、`fn r_1__law_roundtrip()` |
...
1. **位置樹**:願景一行 → 每條需求一行(Law 成立 / 未成立 / 未知)→ 每個目標一行(優先、Law、完成度)→ 每條里程碑一行(達成 / 進行中 / 未開工)與每條調整一行(達成 / 進行中 / 待修訂)→ 綁定的 feature → 目前文檔展開到 step 與 law,各標簽名在不在、law 綠不綠;引用到的 abstract 掛在它底下;沒被任何里程碑綁定的 feature 掛在最後的「沒有目標」底下。目前節點標 `◀ 目前`。全部寫全名。
2. **完成度**三行:需求與目標(最高優先還沒達成的目標 O-n 完成度 x% · 里程碑達成 n / m · 它的需求 R-n Law 成立與否)、產品(feature 達成 n / m · 文檔達成 n / m · 待實作 step n)、本次(目前文檔簽名 m / n · laws g / k)。數字只來自 `devflow status` 與實際跑過的測試;沒跑寫「沿用 <哪一次>」或「未跑」。
3. **主軸檢查**:本次對應哪條需求、哪個目標的哪條里程碑或調整、哪份文檔的哪個 step 或 law;偏離清單(做的事不在任何里程碑的綁定裡、調整動到了里程碑沒綁過的 feature、比最高優先目標的里程碑先做了低優先的、簽名與 Steps 不符、內層 import 外層、測試後門、未登記檔案、open GAP、動了文檔沒寫的東西、兩份 feature 開始寫同一段而沒走 refactor),每條附位置與建議;沒有寫「無」。
4. **下一步**:一條具體命令(參數寫全名),附下面四題的答案;最多兩條替代,各附同四題,外加一句為什麼不是第一。下一步必須從樹上推得出來。
   ...
   - **全部正常時**:每份文檔達成、每條需求 Law 成立、測試全綠、沒有 open GAP、警訊為空,明寫「目前功能全部正常運作,沒有非做不可的事,可以加新功能」,下一步是 `devflow requirement add`(新需求)或 `dev-flow:objective`(既有需求底下的新目標、里程碑或調整)再 `devflow claim feature <slug> --milestone <M-n>`;不另造下一步。
