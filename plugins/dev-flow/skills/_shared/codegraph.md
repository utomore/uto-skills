...
| | skill | 規則 |
|---|---|---|
| **必用** | `/spec-design` | 設計階段一定要用圖查清楚既有符號與影響面再落筆;圖跑不了(專案沒建過、產生器不支援這個語言、`extract` 失敗)時退回一般搜尋,並在收尾寫**具體原因**。**唯一豁免:修訂模式且這次的 gap 沒有指到任何既有符號**(純文字歧義)——圖答不出「這句話是什麼意思」,跳過並在收尾寫一句。gap 一碰到簽名 / 型別 / 呼叫端,豁免失效 |
...
- **可以**用它找候選、算影響面、看全景
- **禁止**拿它當相依性查證的依據。`/spec-design` 的「必須打開原始碼讀到實際定義」一字不改——圖給你檔案與行號,你還是要打開檔案讀到簽名原文
