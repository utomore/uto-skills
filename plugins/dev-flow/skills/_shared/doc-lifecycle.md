...
- 編號**三位數**遞增,取同前綴的最大編號 +1。**「最大編號」要跨分支與 worktree 去問,不能只看當前工作區**:

  ```
  node "<arch-audit skill 目錄>/scripts/scan-ids.mjs" .design          # 全表 + 每組下一個可用號
  node "<arch-audit skill 目錄>/scripts/scan-ids.mjs" .design --next   # 只印下一個可用號(配號時用)
  node "<arch-audit skill 目錄>/scripts/scan-ids.mjs" .design --fetch  # 連遠端分支一起(會連網)
  ```

  它取三個來源的聯集:當前工作區、`git worktree list` 每個 worktree 的 `.design/`(**未 commit 的平行工作只有這一條看得到**)、每條本地與已 fetch 遠端分支的樹。exit 1 = 掃到撞號。

  **為什麼不能只掃資料夾**:工作區只看得到當前分支已 checkout 的檔案。另一條分支已經鑄出 `G-C003` 還沒 merge、或另一個 worktree 正在寫 `G-C003` 還沒 commit,你都看不到,於是也鑄一個 `G-C003`。兩份 `G-C003-<不同 slug>.md` 是**不同檔名**,merge 時 git 不衝突,兩個都會落地——這是**靜默**撞號,要等到有人引用 `G-C003` 才發現它指向兩份文檔。

  **全域(`G-C` / `G-E` / `G-B`)與 `ADR` 一律要跑這支腳本**:它們是全專案共用一組計數器,只要有兩條線在跑就會撞。子系統內的 `F` / `E` / `B` 是每個子系統各一組,不同子系統平行做永遠不會撞——但**同一個子系統有多條分支或多個 worktree 在動時,一樣要跑**。

  掃不到的最後一個縫是**別台機器上還沒 push 的工作**,沒有任何本地掃描看得到它;那個縫只能靠 merge 後再跑一次腳本(它同樣會把落地的兩份同號文檔標成撞號)。
