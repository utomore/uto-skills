...
派工狀態五種,由腳本從 `depends-on`、gap、契約六欄、spike 的 `feeds`、ASM 的 `裁決:` 欄推出來,和 `status` 是兩回事(一份 specced 可能卡在 gap,一份 planned 可能契約齊全能直接委派):**可開工**(派人)、**半步可開工**(只差自己文檔內的東西,先補契約)、**卡住**(被 gap 或未完成的上游擋住)、**等決定**(open spike 或未裁 ASM 在等開發者)、**未開工**(名冊上未建檔的子系統、planned 模組群)。平行判準:不同子系統或不同模組群直接可平行;同模組群看 `modules` 欄有沒有交集,有就串行,沒有就標「要人判」。
