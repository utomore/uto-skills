...
- **結案就刪**:`spec-gaps.md` 只裝 open 的條目,結案 = 寫 REV 並刪條目、檔案空了刪檔。標了 `resolved` 或帶「修訂」行還留著的條目是墓碑,腳本列進提示(格式類)「結案就刪,REV 已承接」,舊專案用 `migrate-v3.mjs --apply` 清;結案有沒有真的改到 spec,看那份 spec 的 REV「依」欄
...
- **§4 等決定的**:open 的 spike(每一份都是一個還沒答完的「要跑了才知道」的問題,它的下游決定正在等證據)與還在檔上的 ASM(契約級假設;條目存在就代表還沒裁,程式碼已照「暫採」蓋上去、沒有人簽過)。逐條轉達全名與「影響誰」;`feeds` 是空的 spike 腳本會明說「不擋任何線」,排最低或建議標 `dropped`。ASM 帶著已填的「裁決」欄還留在檔上的是舊格式墓碑,腳本列提示「裁完就刪」,不算未裁。`concluded` 而 `feeds` 是空的會落到 §6——結論沒有下游
...
6. **待確認假設**(文檔有這一段時,代表本 feature 由 `/subsys-build` 委派產出):逐條檢查每個假設在程式碼裡實際採取了什麼、是否與 Level 2 契約牴觸、是否需要升級成正式的契約決定。**委派產出的文檔要優先看這一段**——它就是「沒有人在旁邊時 AI 自己做的判斷」清單。
   ...
   **ASM 條目存在就是還沒裁**(裁完當場寫 REV 並刪條目,理由見 `../_shared/delegation-design.md`):
   ...
   - 條目帶著已填的「裁決」欄還在檔上 → 舊格式墓碑,列為發現:結論搬進契約修訂行 / 不可逆決定,寫 REV,刪條目
