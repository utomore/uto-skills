...
- **進行中的 spike**(`status: open`,有 `.design/spikes/` 才出現):每一份都是一個還沒答完的「要跑了才知道」的問題,它的下游決定(ADR、契約、feature 的不可逆決定)正在等證據。逐條轉達文檔全名(`SPK-003-storage-engine`)與它的「要回答什麼」,下一步是把那份 spike 做完(`/spike`)或標 `dropped`。`concluded` 而 `feeds` 是空的會落到不一致——結論沒有下游
...
node "<S>/arch-audit/scripts/scan-status.mjs" .design --doc <文檔全名>  # 單一文檔:歸屬 / 介面 / 契約 / 正反向依賴
...
6b. **過程章節有沒有退場**(只查 `status: done` 的文檔,規則見 `../_shared/doc-lifecycle.md`「`done` 的收束」):還留著 `## 待確認假設`、`## 閘門裁決紀錄`、`## 修訂記錄`、`## 已定案的委派判斷` 的,一律列為發現,附「該搬去哪」。**最該點名的是墓碑**——條目本身寫著「已裁決 / 已失效 / 不再適用」卻還留在原位:它同時佔著版面又宣告自己無效,而下一個只讀得到這一份檔的讀者(包括委派出去的 subagent)沒有辦法質疑它。留著不會讓任何檢查變紅,所以只有這一關看得出來
