...
const CONVENTION = {
  ...
  kids: [
    ...
    },
    {
      ...
      kids: [
        { label: "F001 / E001 / B001", meta: "任務文檔 id · 子系統內各一組 · 建檔時掃描 · 永久", note: "永不簡寫:E001 不准寫成 E1" },
        { label: "G-E001 / G-B001", meta: "跨子系統的全域優化 / 修復 · 全域各一組 · 永久" },
        { label: "LAW-1", meta: "行為性質 · 單一 spec 檔內 · spec 角色配 · 永久", note: "qa 一比一翻成 property test" },
        { label: "REG-1", meta: "回歸 law(enhance 專用)· 單一 spec 檔內 · 永久", note: "保護改完必須一模一樣的現有行為" },
        { label: "EX-1", meta: "example · 單一 spec 檔內 · spec 角色配 · 永久", note: "qa 一比一翻成 example test" },
    ...
    },
    {
      ...
      kids: [
        { label: "WAVE-1", meta: "波次 · 單一次展開內 · 編排者算出來 · 用完即棄", note: "拓撲排序的結果,重跑會重算" },
...
function buildProjectTree(designDir) {
  ...
  for (const slug of roster) {
    ...
    const kids = [];
    const idLine = (arr, kind) =>
      arr.length ? { label: `${kind}  ${arr.length} 份`, meta: arr.map((f) => f.split("-")[0]).join("、"), kids: [] } : null;
    for (const n of [idLine(feats, "F"), idLine(enh, "E"), idLine(bugs, "B")]) if (n) kids.push(n);
    ...
    if (law + ex + asm)
      kids.push({ label: "spec 條文", meta: `LAW ${law} 條 · EX ${ex} 個${asm ? ` · ASM ${asm} 條假設` : ""}`, kids: [] });
    if (legacy)
      kids.push({
        label: "⚠ 舊版模板",
    ...
    if (existsSync(gapPath)) {
      ...
      kids.push({ label: "GAP", meta: `${countIds(g.body, "GAP", "G")} 條,未結 ${open}`, kids: [] });
    }
    if (existsSync(join(dir, "build-log.md"))) {
      ...
      kids.push({
        label: "build-log",
        meta: `跑過 /subsys-build · 波次 ${waves} 個 · DEC ${dec} 條 · 上閘門的假設 ${asmRows} 條`,
        kids: [],
      });
    }
