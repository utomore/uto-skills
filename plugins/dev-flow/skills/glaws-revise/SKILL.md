---
name: revise
description: dev-flow(有 .design/ 的專案)的修訂:既有文檔的行為、簽名或 law 要改,回答 GAP,落地調整 RF-n,或經批准的全域 Law 變更;先攤影響範圍與選項,開發者選了才改原檔、寫 REV,再自動接上 build。觸發詞:修訂、改契約、改 spec、改介面、改行為、回答 gap、重開、改 law、放寬 law、改全域 Law、領域不變量、效能優化。Use when an existing document or a global Law must change, a GAP was answered, or a refinement is applied.
user-invocable: true
