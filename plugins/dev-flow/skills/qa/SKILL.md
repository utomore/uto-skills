...
上面這幾段(一份輸出切成幾段,每段一道指令)是載入 skill 時跑 `devflow brief qa <目標> [--root <工作樹>]` 的輸出:規章的節、目標文檔、逐條狀態、Steps 上每條簽名與型別的宣告(目標是 `R-n` / `INV-n` 時是那條需求的驗收、領域不變量的三行,需求的驗收另有它引用到的文檔的 Steps 與宣告)、最內層、這個專案的測試怎麼寫。開工要的全部在這裡,不再另外讀規章、找宣告、翻別的測試檔;宣告那一塊只有簽名與型別,本體不在裡面,也不去開。第一行是指紋,回報的第一項照抄。
