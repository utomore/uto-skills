...
| 輸入 | 產出 |
|---|---|
| 開發者要得到什麼、既有的需求與 feature、專案根目錄 `CLAUDE.md` 的名詞表 | `system.md`「語言與工具」一行優先各級的宣告;`CLAUDE.md`「## 名詞」節裡新講定的名詞與改過的定義;以及 `requirements/` 裡一條需求一個檔 `R-n-<slug>.md`:一句話、驗收、優先、照先後排好的里程碑(全名 `M-n-<slug>`,綁定欄填它做出來的 feature、或它靠修訂達成的既有 feature,還沒有切片的留「-」) |
