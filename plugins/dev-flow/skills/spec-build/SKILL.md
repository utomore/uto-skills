...
6. 修訂目標(`rev > 0`)時另外確認:最後一條 REV 的「重委派」欄列了哪幾條 law 與介面——那就是這次委派的**全部**範圍;「保護」欄列的每一條 `REG-` 回歸 law 都在檔裡嗎?沒有回歸 law 的「行為不變」等於沒有保護;`rev` 與 `## 修訂記錄` 的條數對得上嗎(對不上先回 `/spec-redesign`)
