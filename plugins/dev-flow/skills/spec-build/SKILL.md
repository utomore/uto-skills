...
5. 目標 spec 帶「待確認假設」段落時(委派產出的文檔):先做**層級複審**(`boundary-rules.md` 的兩問:出現在邊界上?改錯驚動其他模組?)。實作層級的不上桌——記進所屬子系統 `build-log.md` 的「自裁清單」(沒有 build-log 就列在收尾回報),出處標「編排者降級」;契約層級的**一次一條**裁決,禁止打包成表:每條先輸出完整發問格式(背景與已查過、選項與各自的當下成本/三個月後代價、subagent 的傾向、可逆性),再用 AskUserQuestion 問這一條,最難逆的先問。呈報**用完整句子,禁止只用術語縮寫**:每個關鍵取捨都要說清楚這是什麼、為什麼要改、不改會怎樣、改了放棄什麼;識別字保留原文,但它做什麼要用句子帶出來。裁決要動 spec 原文或 `design.md` 的,你不改——那就是下面的「要改 spec」;裁決為接受的,記進 build-log 或回報
