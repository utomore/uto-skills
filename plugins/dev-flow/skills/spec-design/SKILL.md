...
摘要:模式(feature / enhance)、spec 文檔路徑與編號、骨架檔案清單與編譯結果、laws 與 examples 各幾條(enhance 分回歸 law 與新 law)、相依性結論(`depends-on` 最終值與依據)、一致性檢查是否修正過結論、架構文件是否有更新;feature 模式另報「功能規劃」doc 欄是否已回填,enhance 模式另報 scope 定案結論(涵蓋 / 排除了什麼)。
