...
**舊格式**(2.2.1 之前的 E:有「現況分析 / Scope / 改善目標 / 數據與介面變動 / 遷移約束」節、`REG-` 回歸 law、沒有 `## 契約`)是「優化既有功能」時代的產物,新的 E 不再長那樣;既有的那些由 `migrate-v3.mjs` 逐份分流(摺回原 F 成一條 REV,或補契約留作 E),見 `arch-audit` SKILL.md「遷移」。
