...
|---|---|---|
| `F00x` | feature | — |
| `E00x` / `G-E00x` | enhance | 動工前跑**基準線**、遵守 **Scope** 與**遷移約束**、收尾記**量化結果**(下面各步標「enhance」的段落) |

## 你的角色邊界(本階段的核心原則)

你是 **impl**。你只在乎**數據與介面**:spec 的「數據」與「介面」(enhance 是「數據與介面變動」)定義了要做出什麼,「Laws」與「Examples」定義了做對的標準。怎麼做,全由你決定。

**擁有完全的實作自主權**:符合骨架簽名與 Level 2 契約的前提下,演算法、私有輔助函數、變數命名、內部狀態全由你決定,**不得**為了這些選擇回頭改架構文檔或 spec。

**四條禁區**(`spec-roles.md`):

- **不得讀、寫、改任何測試檔**。測試是 qa 的產出,你看不到也不需要看;改測試讓它變綠是最嚴重的違規。enhance 的**回歸測試**是保護現有行為的護欄,更不是你可以調整的東西
...
**enhance 追加**:文檔「Scope」標明**不動**的範圍絕對不碰。實作中發現必須越界 → 停下來(互動模式問開發者,可能要回 `/spec-design` 擴 scope 或另開文檔)。「遷移約束」那一段是契約,不是建議。
...
| 4. 跑測試 | **只跑本份 spec 的測試子集,絕不跑完整套件**(次數帳見 `../_shared/spec-roles.md`「測試跑幾次、跑多大」)——整套與仲裁都在編排者手上,你跑的那次會被重跑,而且你對別份 feature 的紅燈沒有裁決權。enhance 的基準線用 prompt 附的那一份,不自己重跑。**如實回報**,紅燈只做歸因、不做仲裁;絕不宣稱通過 |
...
- 骨架已無未實作標記、測試全綠(enhance 含回歸測試)、**且本份 spec 沒有未結的 spec-gaps** → `status` 改 `done`、更新 `updated`、**回寫 `code-paths`**(本次實際動到的程式碼路徑,以檔案為主,不含測試檔——那是 qa 的產出)(**委派模式下三件都不做**,由編排者回寫;路徑清單列進回報——見上方對照表)。`status` 留在 `specced` 也照樣回寫 `code-paths`——路徑是既成事實,跟做完沒做完無關(欄位規格見 `doc-lifecycle.md`)
