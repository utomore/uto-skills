...
```
-- F002 · spec 對照
-- L1 rotate 具冪等性              → prop_rotate_idempotent
-- L2 refresh 後舊 token 必失效    → prop_refresh_invalidates_old
-- E1 正常換發                     → test_refresh_happy_path
-- E3 過期 token                   → test_refresh_expired
```

每條 law 與 example 都必須出現在表上;spec 有、表上沒有 = 交付不完整。
...
**該紅卻綠的測試一律退回重寫**:那代表斷言恆真、或根本沒呼叫到受測介面。**該綠卻紅的回歸測試**代表你對現況的理解與程式碼不符——但你不准打開實作去看,改記 spec-gaps 請設計補明現況行為。

把跑出來的結果如實貼進回報(幾條、紅綠分佈、哪幾條不符預期)。**不得宣稱沒跑過的結果。**
