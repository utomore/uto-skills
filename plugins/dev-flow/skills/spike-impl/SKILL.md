...
- **專案的第一條切片單獨走完**（`roles.md`「分支」）：`system.md`「全域 Law」三個小區都還是空的（領域不變量「無」、層表與對外 I/O 表沒有列），而別條切片已經在建構中（`devflow status`「今天能開幾條線」印的是「專案的第一條切片單獨走完：build/<鍵> …」那一句，這條里程碑不在能開的線裡）→ 停，等那一條抽出全域 Law 並合進主線，再開這一條。三個小區都是空的、也沒有別條切片在建構中 → 這一條就是第一片，照常開工。
...
2. **貫通**：由外而內打通一條最短的路，再由內而外補實。簽名、步驟怎麼拆、資料結構，邊做邊定，不先寫文檔。取型別名、函式名時照名詞表（`.design/vocabulary.md`）上的名詞（表上叫「訂單」的東西，型別就叫 `Order`，不另外發明一個叫法）；名詞表只讀不寫。需要假的就假（假資料、寫死的值、沒接上的外部系統），**每假一處就在決策紀錄「Faked / Unverified」記一列**。
