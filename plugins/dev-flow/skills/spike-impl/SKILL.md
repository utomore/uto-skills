...
| 輸入 | 產出 |
|---|---|
| 一條里程碑的全名 `M-n-<slug>`;它的需求(一句話與驗收);全域 Law(`system.md`「全域 Law」區:領域不變量、層的兩條規則、對外 I/O 的信任邊界) | `build/M-n-<slug>` 分支與工作樹上一條跑得通的切片,與 `.design/journal/M-n-<slug>.md` |
