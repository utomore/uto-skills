---
name: spike
description: 可行性驗證(spike)— 讀原始碼答不出來、要跑了才知道的問題,先寫下問題、判準與 timebox,再在專案根目錄 spikes/SPK-00x-<slug>/ 寫拋棄式程式碼驗證,結論寫進 .design/spikes/SPK-00x-<slug>.md 並指明餵給哪份 ADR / 契約 / feature 檔。可多輪疊代(RND-n,模型屋式的 demo 範本)、可由編排者委派、可平行比較多個候選做法。程式碼保留給 spec-design / spec-impl 參考,產品程式碼禁止 import。觸發詞:spike、可行性驗證、試一下、驗證可不可行、PoC、prototype、原型、快速驗證、模型屋、demo 範本、先做個 demo。Use when a design decision cannot be answered by reading code and needs throwaway experimental code with a recorded verdict.
user-invocable: true
---
...
1. **產出是結論,不是程式碼。** 程式碼保留在 `spikes/` 供日後參考,但**產品程式碼與測試禁止 import 它**,feature 的 `code-paths` 也不得指進去。正式的東西一律從 spec 蓋,可以抄做法,不能把模型屋搬進來。`lint-spikes.mjs` 機械查這一條
...
| **候選比較** | 同一個問題有兩條以上做法要比 | 各候選一個子資料夾 `spikes/SPK-00x-<slug>/<候選>/`,結論裡多一張比較表;委派模式下由編排者 fan out,每個 subagent 只寫自己的子資料夾(見下方「候選比較」) |

模型屋與正式屋的界線只有一條:**正式屋一律從 spec 蓋**。模型屋長到多大都還是 `spikes/` 底下的參考,升格的唯一路徑是把學到的東西寫進契約與 spec,再由 qa 與 impl 各自從 spec 投影。
...
腳本掃過所有分支與 worktree 之後配號,**同時建兩樣東西**:`.design/spikes/SPK-00x-<slug>.md`(frontmatter 骨架 + 章節標題)與同名的程式碼資料夾 `spikes/SPK-00x-<slug>/`(在 `.design/` 的上一層,附一份指回文檔的 `README.md`)。印出 `<id>`、兩條路徑與**全名**(`SPK-003-storage-engine`,之後每次提到都用全名)。

接著打開 `templates/spike.md`,把步驟 0 的三樣東西填進 `## 問題` 與第一輪 `RND-1`,補上 `description`。**委派模式下跳過這一步**:號、路徑與 `RND-n` 由編排者指定。

**第一次在這個專案建 spike 時**另做兩件事,之後不必重做:

- `.gitattributes` 加一行 `spikes/** linguist-vendored`(進 git、但不進語言統計)
- 確認專案的建置 / 測試設定**排除** `spikes/`(`tsconfig` 的 `exclude`、`pyproject` 的 `packages`、cabal 的 `hs-source-dirs`、`go.work` 等)。這一項機器判不了,`lint-spikes.mjs` 只查得到 import,查不到建置圖——所以現在就做

## 2. 寫程式碼(每一輪)

在 `spikes/SPK-00x-<slug>/` 底下(候選比較放各自的子資料夾)寫**最少能回答問題的程式碼**。三條紀律:

- **不追求品質,追求可判定。** 沒有錯誤處理、沒有抽象、沒有測試都可以;但輸出必須能對到判準——判準寫「2 秒內」,程式碼就要印出秒數
- **依賴不得寫進專案的套件管理檔。** spike 要裝的東西裝在它自己的資料夾或虛擬環境裡;非得動專案的 `package.json` / `pyproject` 不可時,先停下來確認(那已經是架構層級的選型,不是 spike 該偷偷做的事)
- **每一輪結束時記一次**:`RND-n` 的「結果」欄照實填,附這一輪的 commit sha 與環境(資料量、外部服務、機器)。timebox 到了沒答案照樣記,寫「未達判準,原因:…」
...
`## 結論` 四格填齊:verdict、一句話結論、學到什麼(**三個月後有人想再試一次時要先知道的事**——這一格是整份文檔最貴的部分)、餵給哪裡。然後:

1. frontmatter:`status: concluded`、`verdict` 填上、`feeds` 列出每一份下游文檔的**全名**(`ADR-004-storage`、`auth/F003-session-list`、`G-C001-session#SessionToken`、`auth/design.md`)、`updated` 換今天
2. **下游那幾份文檔由誰改,講清楚**:spec 或契約走 `/spec-redesign`;新 ADR 走 `/system-design` 更新模式;feature 檔的「不可逆決定」段走 `/spec-design`。**你不改它們**——只在回報裡寫「哪一份、哪一格、寫什麼」
3. 決定不做了的 spike 標 `dropped`,一句話寫為什麼
...
它查三件事:文檔與資料夾成對、frontmatter 合規(`concluded` 要有 `verdict` 與非空 `feeds`)、產品程式碼有沒有 import `spikes/`。
...
- 每個候選一個 subagent、一個子資料夾 `spikes/SPK-00x-<slug>/<候選>/`;prompt 帶 `【委派模式】`、問題、判準、timebox、子資料夾路徑
...
| 4. 結案 | **不寫 spike 文檔、不改 frontmatter、不 commit**——由編排者單線寫。回報固定五項:verdict、對每條判準的觀察結果(數字或現象)、程式碼路徑與 sha、環境、**沒驗到的**(判準之外、這次沒碰的) |
...
- **不當第二條開發路徑**。模型屋做得再大,升格只有一條路:寫進 spec,再走 qa ∥ impl
