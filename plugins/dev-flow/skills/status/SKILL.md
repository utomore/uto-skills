...
3. **講人話**:不重印報告;先講需求與全域 Law,再照八段講「今天該做什麼、為什麼」。每段一到三句,文檔與里程碑都寫全名(`F-001-checkout`、`M-1-checkout`);警訊那張表照抄怎麼辦欄。第 1 段的線可以同時各開一波(`roles.md`「分支」):還沒有切片的里程碑是 `dev-flow:spike-impl`,`ready` 的文檔是 `dev-flow:build`;建構中的講它在哪條分支、走到哪一步、下一個動作是誰的(切片完成等 `dev-flow:scope-laws`、Law 討論中等開發者拍板與全域的候選、達成等 `dev-flow:integrate`);報告附了共用檔案提示就照講。要改既有文檔的線、待修訂的里程碑與 open 的 GAP,都照 `laws.md`「分流」那張表講該走哪一個 skill。
   - **需求**(從報告的需求表與每條需求底下的里程碑講):每條需求是已驗收、待審核、待重審、建構中、未知還是未達成,與判定來源。**待審核的要當面請開發者驗**:機器只出證據,達成與否只有開發者判得了(`features.md`「需求的達成只有人判得了」)——把報告第 3 段那條需求的證據、怎麼驗的每一步唸給他聽,請他跑過、當場說認不認;認了才替他跑 `devflow requirement accept R-n --by <他的 email> --evidence "<他說的那一句>"`,**不准代他認、不准整批認**。待重審的講清楚是哪一份證據翻掉了。「上線」欄講使用者拿到了沒(`tooling.md`「發布」):已驗收而還沒上線的,照報告第 3 段講是哪幾份文檔的最新 commit 還沒進發布的 tag,發不發布由開發者決定,不替他打 tag。里程碑全部達成而驗收測試沒過的(里程碑切漏了,或驗收寫錯,回 `dev-flow:require-design`),明講「驗收沒過」;驗收寫了三行卻沒有驗收測試的,講建議路線裡那條 `dev-flow:build R-n`。需求表的「依賴」欄講哪條需求疊在哪條上面(從文檔的引用推的,不是人寫的):互不依賴的可以同時開工,有依賴的,被依賴的那條沒達成,依賴它的證據就還不齊。最高優先還沒達成的需求是哪一條、里程碑達成幾條、它的下一條里程碑卡在哪份文檔、還沒有切片、還是待修訂(靠修訂達成、還沒有 REV 引用它:講它綁哪一份、走哪一個修訂的 skill);有沒有 feature 沒被任何里程碑綁定、有沒有需求沒有里程碑;正在做的事是不是最高優先需求的下一條里程碑,不是就明講;願景還是模板就先講這件事。答的是「必須達成的事達成了沒、我們有沒有朝向需求」。
