...
**閘門停不停,由步驟 2 選定的檔位與本波的議程共同決定。** 三道機械過濾(降級 / 沿用 / 合併)跑完之後,先數議程:不可逆決定、未宣告的新增依賴邊、**跨 feature 對帳對不上的名字**、契約層級假設各幾條,加上你自己讀完的疑慮。**議程為 0 且你沒有疑慮**時(「標準」數四類、「快速」只數前三類),該檔位**降級為非阻塞呈報**——照發第 (1) 段的總覽、在 build-log 記一行「W2 空議程自動放行」、直接進 3d,不要問。此時的閘門問的是「你同意這份沒有任何爭議點的 spec 嗎」,那不是審查,是蓋章;而蓋章慣了,有爭議點的那一波也會被順手蓋過去。
