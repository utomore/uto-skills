...
3. 彙整所有「待確認假設」與「建議修改的 Level 2 契約」,**先不要自己改契約**。彙整時做**雙向層級複審**——兩個方向都要跑,只做降級是危險的過濾:
   ...
   真正契約層級的才留給 3c 逐條裁決,每條的選項/代價/可逆性欄位必須齊全,缺的退回該 feature 的 spec subagent 補寫
...
7. 待裁決的契約假設**數量**與一行標題清單。**超過 5 條就先說破**:這通常代表契約卡本身留了太多洞,問開發者是逐條裁完,還是回 `/subsys-design` 補契約卡後重跑本波受影響的 spec——十幾條逐條裁,常常不如把卡補好一次
...
3. **契約層級的待確認假設**(第 7 點),按**可逆性排序,最難逆的先問**(注意力最新鮮時裁最貴的決定)
