...
3. **契約層級的待確認假設**(第 7 點),排序看兩個維度:**先按可逆性,最難逆的先問**;可逆性相同時,**波及的 feature 數多的先問**。一個難逆但只影響一個模組內部的決定,和一個可逆但五個 feature 都要照著寫的決定,後者值得更清醒的注意力——所以呈報每一條時都要把「這條影響 F003 / F005 / F007」講出來,那是開發者判斷這題該花多少心思的依據
...
- **發出任何委派之前,先記下 `HEAD` 的 sha**(骨架快照)寫進 build-log「排程」表的本波那一列——3e 要拿它驗 qa 的紅綠
...
- 有阻塞或新增 spec-gaps → **立刻停下本波的後續 impl**,不要繼續往下堆(誤差沿依賴鏈複利是本流程最大的風險),先把 gap 寫進檔案,再進 3e
...
5. 向開發者回報,固定五塊:
   ...
   - **契約裁決與自裁**:閘門裁過的**不可逆決定**、**新增依賴邊**與**契約假設**各幾條、裁了什麼、回寫到哪;自裁清單本階段新增幾條、其中編排者**降級**幾條、**升級**幾條。降級比例高 → spec subagent 把實作細節往上丟,門檻要收緊;升級比例高 → 它的層級自答太寬鬆,`觸及符號` 欄要求要講得更死。邀開發者從自裁清單**抽 2-3 條**看裁得對不對——這是雜訊的事後審計路,裁錯率高再收緊,比預設全部上桌便宜
