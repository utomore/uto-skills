...
3. 彙整所有「待確認假設」與「建議修改的 Level 2 契約」,**先不要自己改契約**。彙整時做**層級複審**:對每條假設重跑 `boundary-rules.md` 的兩問(答案出現在邊界上嗎?改錯要驚動其他模組嗎?)。subagent 判錯層級、其實是實作層級的 → **降級**,不上閘門,連同各 subagent 回報的「自裁記錄」一起記進 build-log 的「自裁清單」(出處標「編排者降級」);真正契約層級的才留給 3c 逐條裁決,且每條的選項/代價/可逆性欄位必須齊全,缺的退回該 feature 的 spec subagent 補寫
...
2. 骨架檔案清單與編譯結果
3. 你自己讀過之後的疑慮:哪條 law 讀起來有兩種解釋、哪個介面沒有被任何 law 或 example 覆蓋
4. 自裁清單:只報「本波自裁 N 條(其中編排者降級 M 條),已記進 build-log」——**不逐條唸**,那是抽查的材料,不是裁決的議程
5. 待裁決的契約假設**數量**與一行標題清單。**超過 5 條就先說破**:這通常代表契約卡本身留了太多洞,問開發者是逐條裁完,還是回 `/subsys-design` 補契約卡後重跑本波受影響的 spec——十幾條逐條裁,常常不如把卡補好一次

**(2) 契約假設逐條裁決(一次一條,禁止打包成表)**:

按**可逆性排序,最難逆的先問**(注意力最新鮮時裁最貴的決定)。每一條:先輸出完整發問格式(背景與已查過——從 spec 的假設條目取材;選項與各自的當下成本/三個月後代價;subagent 的傾向與理由;可逆性),再用 AskUserQuestion 問**這一條**,選項就是假設列的方案(subagent 的傾向放第一個並標明)。等裁決落地才進下一條。
...
**(3) 回寫與契約 diff 確認(批准拆兩段)**:裁決中要回寫 `design.md`(契約補完)或 spec 條文的,由**你**回寫。「同意這個方向」和「同意這段文字成為契約」之間有翻譯損耗,而契約是唯一真相——回寫完成後,把每一處改動用**原文 → 新文**對照一次呈報,開發者確認措辭(有意見就改了重呈)後才 commit(message 例:`docs(<slug>): W2 契約裁決回寫`),並把裁決與回寫位置記進 build-log 的「待確認假設彙總」。
...
5. 向開發者回報,固定五塊:
   ...
   - **契約裁決與自裁**:閘門裁過的契約假設摘要(幾條、裁了什麼、回寫到哪);自裁清單本階段新增幾條、其中編排者降級幾條——降級比例高就點名(spec subagent 的層級判斷在漂,建議收緊門檻)。邀開發者從自裁清單**抽 2-3 條**看裁得對不對——這是雜訊的事後審計路,裁錯率高再收緊,比預設全部上桌便宜
...
- **自裁清單**:總條數與降級條數,提醒開發者定期抽查(抽 2-3 條;裁錯率高就收緊層級門檻)
