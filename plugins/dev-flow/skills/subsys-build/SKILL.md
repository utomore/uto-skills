...
- **發出任何委派之前,先照 `../_shared/spec-roles.md`「骨架快照」建好快照 worktree**,sha 寫進 build-log「排程」表的本波那一列——3e 要拿它驗 qa 的紅綠
...
- **qa 與 impl 之間不設先後**:檔案集不相交(測試檔 vs 實作檔),**寫入**不會互蓋。但**讀取會**——qa「該紅卻綠」判準讀的正是 impl 填的那個狀態,impl 先落地判準就靜默失效。所以紅綠的最終確認不靠誰先跑完,移到 3e 由你在骨架快照上做(見 `../_shared/spec-roles.md`「骨架快照」)
...
**(0) 驗 qa 的紅綠基線**:照 `../_shared/spec-roles.md`「骨架快照」,在快照狀態上跑本波 qa 的測試檔(不必跑整套),逐條對交付判準那張表與 qa 對照表的預期欄。該紅卻綠的,退回該 feature 的 qa 重寫(`dev-flow:spec-qa`,`model: "sonnet"`,只給該幾條測試名與對應的 spec 條文,**不得附上實作原始碼**),重寫後重驗。**這一步一定要在跑整套之前做**:整套一綠,假綠與真綠就再也分不開,而假綠不產生紅燈,仲裁協議永遠不會為它啟動。驗不成的照 spec-roles 標「未驗證」帶到閘門,不得默認通過。
