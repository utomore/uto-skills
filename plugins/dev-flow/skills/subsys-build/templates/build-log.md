...
> **這張表是索引與合併關係,不是裁決結果的權威。** 裁決落地就是一次修訂:結論寫進該 spec 的契約修訂行 / `## 不可逆決定`(架構級開 ADR),`## 修訂記錄` 加一條 REV(依欄寫「閘門 WAVE-n」),然後刪掉那條 ASM(`../_shared/delegation-design.md`)。本表答的是「哪幾條被併成同一次裁決、沿用了哪一次、回寫到哪裡」——那是編排過程的審計軌跡,`/arch-audit status` 依定義不掃本檔。**兩邊不一致時以 spec 的 REV 與契約為準**,並回頭修本表。
