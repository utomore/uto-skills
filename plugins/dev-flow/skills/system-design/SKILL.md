...
0. **專案模式(第一題,先問這個)**:這是**全新建立**的專案(`greenfield`:所有東西都是第一版,沒有既有使用者、既有資料、既有的對外承諾),還是**維護型**(`brownfield`:已經有人在用)?用 AskUserQuestion 問,答案寫進 frontmatter 的 `mode`。

   **這一題排第一是因為它決定後面整類問題該不該問**:`greenfield` 專案裡,migration、舊格式、既有使用者怎麼辦、要不要留舊版 API——這些問題全部**不准問**,也不准在設計裡預留相容層,決策一律以專案的未來性為第一優先;`brownfield` 則相反,那些是必問。兩個模式的完整規則與唯一例外(外部系統的既有契約改不動,照實承接)見 `../_shared/boundary-rules.md`「專案模式」
...
status: active
mode: greenfield        # greenfield(全新建立)| brownfield(維護型);訪談第 0 題定案,見 boundary-rules「專案模式」
created: <today>
updated: <today>
...
8. **模式一致**:`mode` 欄有值,而且全文與它不牴觸——`greenfield` 專案裡不得出現 migration 章節、相容層、版本欄位、`legacy` 模組或「先支援兩種格式」這類設計;真的需要承接外部系統的既有格式時,寫進「對外 I/O 契約」並註明那是**外部**契約(規格我們改不動),不是自己的舊版
9. **縮寫不撞號**:全文自訂的縮寫與代號(層的名字、里程碑代號、模組代號…)逐一對 `doc-lifecycle.md`「編號與縮寫註冊表」查一遍,**不得與表內任何首碼同形**。專案的縮寫會被之後每一份文檔沿用,撞了號(如拿 `E0` 當階段、`L1` 當層)之後每一次對話都要靠上下文猜它指誰——這種混淆不會報錯,只會讓盤點與審查悄悄對錯東西
