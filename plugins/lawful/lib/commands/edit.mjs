...
export function milestoneVerify(design, fullName, command) {
  if (notMigrated(design)) return { text: NOT_MIGRATED, exitCode: 1 };
  if (!command) return { text: `要一道指令：lawful requirement verify ${fullName || '<M-n-slug>'} "<跑起來看得到這一句的指令>"`, exitCode: 1 };
...
export function requirementAccept(design, reqId, { by = '', evidence = '', date = today() } = {}) {
  ...
  if (!by) return { text: `要寫是誰認的：lawful requirement accept ${reqId} --by <email> --evidence "<憑什麼認的，一句>"`, exitCode: 1 };
  if (!evidence || /<[^>]*>/.test(evidence)) return { text: `要寫憑什麼認的（跑了哪道指令、看到什麼、測試綠幾條）：lawful requirement accept ${reqId} --by ${by} --evidence "<一句>"`, exitCode: 1 };
  const file = requirementFile(design, req);
  const lines = fs.readFileSync(file, 'utf8').split(/\r?\n/);
  ...
  return {
    text: [
      ...
      '人簽的是當時那一份證據：之後驗收測試翻紅、或里程碑被修訂動到，lawful status 會把它列成待重審，重驗過再跑一次這道指令',
    ].join('\n'),
    exitCode: 0,
  };
}
