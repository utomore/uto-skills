...
export function analyze(design, source, adapter, results) {
  ...
  for (const p of design.pipelines) {
    ...
    // 程式碼裡沒有這個宣告（或型別對不上）的 stage:build 對帳那一步會停在它，所以它不是能開的線
    const missing = stages.filter((s) => s.state === '找不到' || s.state === '不一致').map((s) => ({ name: s.name, state: s.state }));
    info.set(p.fullName, { p, stages, sigOk, sigTotal: steps.length, stubCount, obsOk, obsTotal: observes.length, laws, examples, gaps, refs, missing, referrers: [] });
  }
...
// 靠修訂達成的里程碑而還沒有 REV 引用它 = 待修訂；還沒有決策紀錄 = 切片中；有決策紀錄而里程碑還沒綁 pipeline = 切片完成；綁的 pipeline 有 draft = Law 討論中；
...
export function slicePhase(design, source, key) {
  ...
  // 靠修訂既有的 pipeline 達成的里程碑：那幾條 pipeline 還沒有任何一條 REV 引用它，就是待修訂——不看決策紀錄，也不因為 pipeline 本來就 verified 而算做完
  if (m) {
    const maker = makerOf(design);
    const revised = m.binds.filter((b) => maker.get(b) !== m);
    const cited = revised.some((b) => (design.pipelines.find((d) => d.fullName === b) || { revs: [] }).revs.some((r) => r.cites.includes(m.id)));
    if (revised.length && !cited) return '待修訂';
  }
...
// 人工審核過的那一條需求在驗收記錄表的結論欄寫這個字；只由 lawful requirement accept 寫
...
export function verifySteps(design, q) {
  const cmd = design.cone ? design.cone.commands['建置'] : null;
  const build = typeof cmd === 'string' ? cmd : cmd ? Object.entries(cmd).map(([d, v]) => `${d} = ${v}`).join(';') : '';
  const steps = build ? [`\`${build}\``] : [];
  for (const m of q.ms) steps.push(m.verify ? `${m.fullName} \`${m.verify}\`` : `${m.fullName} 沒有怎麼驗欄（lawful:require-design 補一道指令）`);
  steps.push(`看${q.accept ? `「${q.accept.title}」` : '驗收那一句'}成不成立`);
  return steps;
}

// pipeline → 做出它的里程碑：綁這條 pipeline 的里程碑裡，M-n 編號最小的那一條（編號是流水號、永不重排，需求的優先怎麼改都不換人）。
// 其餘綁它的每一條都是靠修訂它達成的；從調整表讀來的列 (RF-n) 一律算靠修訂的。同號時取先讀到的那一條。
export function makerOf(design) {
  const numOf = (m) => Number((/^M-(\d+)$/.exec(m.id) || [0, Infinity])[1]);
  const maker = new Map();
  for (const q of design.requirements.requirements) for (const m of q.milestones) {
    if (m.fromRefinement) continue;
    for (const b of m.binds) if (!maker.has(b) || numOf(m) < numOf(maker.get(b))) maker.set(b, m);
  }
  return maker;
}

// 需求 → 里程碑 → pipeline。
// 一條 pipeline 被好幾條里程碑綁定時，編號最小的那一條 (makerOf) 是做出它的：那條 pipeline 達成就算數。
// 其餘綁它的里程碑是靠修訂它達成的：那條 pipeline 要達成，而且它的修訂記錄裡有一條 REV 的依欄引用了這條里程碑（比對編號）。
// 里程碑達成 = 綁定的每條 pipeline 各照各的規則都成立；里程碑依表上的順序走，全部達成 = 這條需求的建置走完 (built)。
// 里程碑的狀態字：達成；待修訂 = 靠修訂的那幾條 pipeline 還沒有任何一條有引用它的 REV；其餘是進行中。
// 需求達成與否只由人判：機器算的是證據齊了沒（驗收測試綠，或沒有驗收測試而里程碑全部達成），證據齊了印「待審核」等人，
// 人在驗收記錄表上簽了才是「已驗收」；簽過之後證據翻掉是「待重審」。任何情況下 status 都不自己把一條需求算成達成。
// rank 給每條 pipeline 一個排序鍵（需求優先、需求順序、里程碑順序），建議路線與能開的線照它排；沒被綁的排最後。rank 只管排序與標籤，不拿來判誰做出 pipeline。
export function requirementView(design, a) {
  ...
  const maker = makerOf(design);
  const reqs = sorted.map((q) => {
    const ms = q.milestones.map((m) => {
      const docs = m.binds.map((b) => {
        const x = a.info.get(b) || null;
        const byRevision = maker.get(b) !== m;
        const revs = byRevision && x ? x.p.revs.filter((r) => r.cites.includes(m.id)) : [];
        return { name: b, x, byRevision, revs, cited: !byRevision || revs.length > 0 };
      });
      const achieved = docs.length > 0 && docs.every((d) => d.x && d.x.achieved && d.cited);
      const revised = docs.filter((d) => d.byRevision);
      const state = achieved ? '達成' : revised.length && !revised.some((d) => d.cited) ? '待修訂' : '進行中';
      return { ...m, docs, achieved, byRevision: revised.length > 0, state, toRevise: revised.filter((d) => d.x && !d.cited).map((d) => d.name) };
    });
  ...
  // 一份文檔算哪一條需求的：做出它的那條里程碑所在的需求；沒有的（只被調整表的列綁過）退到排序最前的那一條
  const owner = new Map();
  for (const q of reqs) for (const m of q.ms) for (const d of m.docs) if (!d.byRevision) owner.set(d.name, q.id);
  const ownerOf = (name) => (owner.has(name) ? owner.get(name) : rank.has(name) ? rank.get(name).q.id : null);
  for (const q of reqs) {
    const deps = new Set();
    for (const m of q.ms) for (const d of m.docs) {
      ...
      for (const r of [d.name, ...(d.x ? d.x.refs : [])]) {
        const at = ownerOf(r);
        if (at && at !== q.id) deps.add(at);
      }
    }
    q.dependsOn = [...deps].sort((x, y) => Number(x.slice(2)) - Number(y.slice(2)));
  }
  // 一條需求的里程碑照順序走：綁在後面幾條里程碑的 pipeline 等前面那一條達成。
...
// 一條 draft 的 pipeline 的切片在不在：它在某條 build 分支上（切片那一波還沒收），或這棵樹上有它、或綁它的里程碑的決策紀錄。
// 兩者都沒有 = 它是在切片之外 claim 出來的，`lawful:scope-laws` 的前置要決策紀錄，開不了工：該走的是那條里程碑的切片
...
// 每條需求下一條還沒達成的里程碑，如果它還沒綁任何 pipeline、或綁的每一條都是還沒有切片的 draft，就是一條可以開的切片（要有英文名才有分支可開）
// 專案的第一條切片單獨走完：全域 Law 三區都還是空的（沒有領域不變量、四層「裝什麼」那一句一層都還沒寫、對外 I/O 表沒有列）、
// 而且已經有一條切片的分支在建構中，別的切片等它（holdKey 是那條分支的鍵，held 是等著的那幾條）
export function sliceLines(ov, building, design = null, a = null) {
  ...
  const empty = !!cone && !cone.invariants.length && !Object.keys(cone.layerNotes || {}).length && !design.io.length;
...
// 每條需求下一條還沒達成的里程碑，如果它靠修訂既有的 pipeline 達成、而其中還有 pipeline 沒有任何一條 REV 引用它，就是一條待修訂的線（toRevise 是那幾條）
// 已經有分支的（鍵是這條里程碑的全名，或它要修訂的那條 pipeline 的全名）另外列，key 是那條分支的鍵
export function reviseLines(ov, building = new Set()) {
  const next = ov.reqs.map((q) => ({ q, m: q.ms.find((m) => !m.achieved) })).filter((s) => s.m && s.m.toRevise.length)
    .map((s) => ({ ...s, key: [s.m.fullName, ...s.m.toRevise].find((k) => building.has(k)) || null }));
  return { openable: next.filter((s) => !s.key), inBuild: next.filter((s) => s.key) };
}
export const reviseHint = (m) => `lawful:scope-revise ${m.toRevise.join('、')}（既有的 law 不動、新的承諾用新增的 law 表達；要調整既有的 law 就 lawful:scope-laws），REV 的依欄寫 ${m.fullName}`;
...
export function buildKeyOf(x, ov, building) {
  ...
  // 綁它的任何一條里程碑（做出它的那一條，或靠修訂它達成的那幾條）有分支就算
  for (const q of ov.reqs) for (const m of q.ms) if (m.binds.includes(x.p.fullName) && building.has(m.fullName)) return m.fullName;
  return null;
}

// 一條 pipeline 在報告與看板上的同一句狀態
export function docState(x) {
  return x.achieved ? '達成' : x.gaps.length ? `卡 ${x.gaps.map((g) => g.id).join('、')}` : x.missing.length ? `${x.missing.length} 個 stage 對不上程式碼` : x.blockedBy.length ? `等 ${x.blockedBy.join('、')}` : x.heldBy ? `等 ${x.heldBy.m.fullName}` : '進行中';
}
// ready 的 pipeline 有 stage 在程式碼裡找不到或不一致：build 對帳會停在它，宣告由寫這條文檔的 skill 補，不在 build 補
export const missingHint = (x) => `${x.p.fullName} 的 ${[...new Set(x.missing.map((s) => `${s.name}（${s.state}）`))].join('、')} 與程式碼對不上：寫這條的 skill 補宣告（切片那一波 lawful:scope-laws；修訂新增的 stage lawful:scope-revise，本體標成未實作）`;

// build 開得了的 pipeline:ready、沒 open GAP、每個 stage 在程式碼裡（在、搬家或未實作）、引用的 pipeline 全部達成、
...
// 能開 = buildable 的那幾條（消費者在 subflow 合進主線之後才開，roles.md「分支」）
...
export function warnings(design, a, ov, source, adapter, stale = new Set(), inv = invariantView(design, a), glob = globalView(design, a, source, adapter, inv)) {
  ...
  for (const q of ov.reqs) {
    ...
    else if (q.accept.formal && !q.tested) warn(q.id, '驗收寫了三行卻沒有驗收測試，證據不齊，還不能請人審核', `lawful:build ${q.id}（只派 qa 寫一條歸屬 "${q.id}#ACCEPT" 的測試）；不能自動化就只留一句，證據改由里程碑全部達成承接`);
    ...
    if (q.built && q.state === '未達成') warn(q.id, `里程碑全部達成，證據卻不成立（${q.source}）`, '驗收沒過：先查驗收測試；里程碑切漏了就 lawful:require-design 補一條，做錯的改那條 pipeline（要調整既有的 law 走 lawful:scope-laws，既有的 law 不動走 lawful:scope-revise）');
    if (q.state === '待重審') warn(q.id, `${q.signoff.date} 已驗收，證據現在翻掉了（${q.evidence}）`, `先把紅的修回來，再 lawful requirement accept ${q.id} 重簽一次；人簽過的是當時那份證據，不是永久的`);
    if (q.state === '待審核' && q.ms.some((m) => !m.verify)) warn(q.id, `等人工審核，${q.ms.filter((m) => !m.verify).map((m) => m.fullName).join('、')} 沒有「怎麼驗」欄，審的人沒有手段可以跑`, '切片收尾時 lawful:integrate 會把決策紀錄的 Entry 那道指令寫進怎麼驗欄；現在補：lawful requirement verify <M-n-slug> "<指令>"');
    for (const m of q.ms) {
      ...
      if (!m.slug && !m.fromRefinement) {
        if (m.byRevision) warn(m.fullName, '沒有英文名，REV 的依欄寫不出它的全名', `lawful:require-design 把第一格寫成 ${m.id}-<slug>（kebab-case 英文）；引用一條里程碑一律用全名`);
        else warn(m.fullName, '沒有英文名，切片開不了分支', `lawful:require-design 把第一格寫成 ${m.id}-<slug>（kebab-case 英文）；切片的分支 build/${m.id}-<slug> 與決策紀錄以它為鍵`);
      }
...
export function suggestRoutes(design, a, ov, warnCount, building = new Set(), inv = invariantView(design, a)) {
  ...
  // 只列 build 開得了的線（第 1 段那幾條）；等別條的、等前一條里程碑的、程式碼裡缺宣告的都不列，它們在第 2 段
  for (const x of buildable(a, ov)) steps.push(`lawful:build ${x.p.fullName}（${ov.tag(x.p.fullName)}）`);
  // ready 而程式碼裡沒有宣告：build 對帳會停，先把宣告補回來
  for (const x of a.info.values()) if (x.p.status === 'ready' && !x.achieved && x.missing.length) steps.push(`${missingHint(x)}；補上才 lawful:build ${x.p.fullName}`);
  // draft 照需求的優先排；沒有切片的那幾條 scope-laws 開不了工（它的前置要決策紀錄），線是那條里程碑的切片
  const sliceless = slicelessDrafts(a, ov, building, design);
  ...
  for (const x of drafts) {
    if (!sliceless.has(x.p.fullName)) { steps.push(`lawful:scope-laws ${x.p.fullName}（還是 draft：Law 談完、開發者拍板才改 ready，之後自動接上 build）`); continue; }
    const m = ov.rank.has(x.p.fullName) ? ov.rank.get(x.p.fullName).m : null;
    if (m && onSliceLine.has(m.fullName)) continue;
    steps.push(m
      ? `lawful:spike-impl ${m.fullName}（${x.p.fullName} 還是 draft，而這棵樹沒有它的切片：先做出切片，lawful:scope-laws 才開得了工）`
      : `${x.p.fullName} 還是 draft、沒有切片，也沒有里程碑綁它：lawful:require-design 給它一條里程碑再 lawful:spike-impl；用不到就 lawful:scope-laws ${x.p.fullName} 退役`);
  }
  // 靠修訂既有的 pipeline 達成、還沒有 REV 引用它的里程碑：每條需求下一條，照需求的優先排
  ...
  for (const s of slices.openable) steps.push(`lawful:spike-impl ${s.m.fullName}（${sliceTag(s)}）`);
  ...
  for (const q of ov.reqs) if (q.state === '待審核') steps.push(`請開發者親自審核 ${q.id}（${q.evidence}；怎麼驗見第 3 段），認可了才 lawful requirement accept ${q.id} --by <email> --evidence "<一句>"`);
  ...
  if (!steps.length) {
    ...
    else if (allDone && !warnCount) note = '目前功能全部正常運作：每條 pipeline 達成、每條需求都經開發者親自驗收、每條領域不變量成立、測試全綠、沒有 open GAP、沒有警訊。沒有非做不可的事，可以加新功能：lawful:require-design 談一條新需求，或替既有的需求加一條里程碑';
    else if (allDone) note = `目前功能全部正常運作（每條 pipeline 達成、每條需求都經開發者親自驗收、每條領域不變量成立、測試全綠、沒有 open GAP）；警訊還有 ${warnCount} 條，照第 6 段的怎麼辦欄清，清完加新功能`;
...
// 需求、里程碑、io pipeline、pipeline 的總數與達成數：報告的第二行與看板的數字同源
...
export function statusReport(design, source, adapter, results, resultNote, building = new Set(), stale = new Set(), phases = new Map()) {
  ...
  if (!cone) out.push(`- 沒有 Cone.md；${design.legacySystem ? 'lawful migrate cone --write' : 'lawful:kickoff 建它'}`);
    ...
    for (const q of ov.reqs) {
      ...
      // 靠修訂達成的那一條：還沒有 REV 引用這條里程碑是「待修訂」，有了就看 pipeline 自己走到哪
      const state = (d) => (!d.x ? '不存在' : !d.cited ? '待修訂' : d.x.achieved ? '達成' : d.x.gaps.length ? `卡 ${d.x.gaps.map((g) => g.id).join('、')}` : d.x.p.status === 'draft' ? '還是 draft' : '進行中');
      if (next) out.push(`- ${q.id} 下一個里程碑：${next.fullName} ${next.title}${next.docs.length ? `（${next.docs.map((d) => `${d.name} ${state(d)}`).join('、')}）` : (next.slug ? `（還沒有切片：lawful:spike-impl ${next.fullName}）` : '（還沒有切片，也還沒有英文名）')}${next.toRevise.length ? `；待修訂：${reviseHint(next)}` : ''}`);
    }
  }
  const unbound = [...a.info.keys()].filter((n) => !ov.rank.has(n));
  ...
  for (const q of ov.reqs.filter((x) => x.state === '待審核' || x.state === '待重審')) {
    ...
    out.push(`  - 認可：lawful requirement accept ${q.id} --by <你的 email> --evidence "<憑什麼認的，一句>"`);
  }
