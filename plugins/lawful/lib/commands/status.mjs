...
export function slicePhase(design, source, key) {
  ...
  if (!docs.length) return design.journals.some((j) => j.key === key) ? '切片完成,等 lawful:law-design' : '切片中';
...
// 每個目標下一條還沒達成的里程碑,如果它還沒綁任何 pipeline,就是一條可以開的切片(要有英文名才有分支可開)
...
export const sliceTag = (s) => `${s.o.id} 優先 ${s.o.priority || '?'} · ${s.m.title}`;
...
export function warnings(design, a, ov, source, adapter, stale = new Set(), inv = invariantView(design, a), glob = globalView(design, a, source, adapter, inv)) {
  ...
  if (!cone) warn('Cone.md', '不存在', design.legacySystem ? 'lawful migrate cone --write' : 'lawful:project 立案');
  else {
    if (cone.visionState === 'missing') warn('Cone.md', '沒有 ## 願景 節', 'lawful:project 訂願景');
    else if (cone.visionState === 'template') warn('Cone.md', '願景還是模板', 'lawful:project 訂願景');
    if (cone.requirementsState === 'missing') warn('Cone.md', '沒有 ## 需求 節', 'lawful:project 訂需求');
    else if (!cone.requirements.length) warn('Cone.md', '沒有任何需求', 'lawful requirement add <一句話> --accept <句>(至少一條)');
    ...
    if (cone.requirements.some((q) => (q.accept && q.accept.written === 'Law') || q.implied) || design.objectives.objectives.some((o) => o.lawItem)) warn('Cone.md', '需求或目標檔寫著「- Law:」或蘊含說明;需求附的是驗收,目標沒有 Law', 'lawful migrate laws --write');
  }
  for (const q of ov.reqs) {
    if (q.placeholder) warn(q.id, '需求還是模板', 'lawful:project 寫成一句話');
    if (!q.accept) warn(q.id, '沒有驗收', 'lawful:project 補一句可判定的話,寫成「- 驗收:…」');
    else if (q.accept.placeholder) warn(q.id, '驗收還是模板', 'lawful:project 寫成可判定的一句');
    else if (q.accept.formal && !q.tested) warn(q.id, '驗收寫了三行卻沒有驗收測試,達成與否未知', `lawful:build ${q.id}(只派 qa 寫一條歸屬 "${q.id}#ACCEPT" 的測試);不能自動化就只留一句,由建置路線推`);
    if (!q.objectives.length) warn(q.id, '沒有任何目標', `lawful objective add <slug> <一句話> --requirement ${q.id} --priority <1-4>`);
    if (q.built && q.holds === false) warn(q.id, `建置路線全部達成,需求卻未達成(${q.source})`, '驗收沒過:先查驗收測試,缺的能力開里程碑(lawful:objective),做錯的走 lawful:revise');
  ...
  for (const v of inv) {
    ...
    else if (!v.law.formal) warn(v.id, '還沒有三行式,成立與否未知', 'lawful:law-design 在 types 層的型別出現後把它寫成三行(識別字只用 types 層的匯出與型別名)');
  ...
  for (const o of ov.objs) {
    ...
    if (!o.ms.length) warn(o.id, '沒有任何里程碑', 'lawful:objective 補里程碑(lawful objective milestone <O-n> <slug> <一句話>)');
    for (const m of o.ms) {
      if (seenM.has(m.id)) warn(m.id, '里程碑編號重複', '編號全資料夾唯一;配號只走 lawful objective milestone');
      seenM.add(m.id);
      if (m.placeholder) warn(m.id, '里程碑還是模板', 'lawful:objective 寫成一句話');
      if (!m.slug) warn(m.id, '沒有英文名,切片開不了分支', `lawful:objective 把第一格寫成 ${m.id}-<slug>(kebab-case 英文);切片的分支 build/${m.id}-<slug> 與決策紀錄以它為鍵`);
      for (const d of m.docs) if (!d.x) warn(m.id, `綁定的 ${d.name} 不存在`, '改成 pipelines/ 裡有的全名,或刪這個綁定');
    }
    for (const rf of o.rfs) {
      if (seenRf.has(rf.id)) warn(rf.id, '調整編號重複', '編號全資料夾唯一;配號只走 lawful objective refinement');
      seenRf.add(rf.id);
      ...
      for (const n of rf.outside) if (!rf.missing.includes(n)) warn(rf.id, `動到的 ${n} 不在 ${o.id} 任何里程碑的綁定裡`, '優化路線不引入新 pipeline:只動本目標建置路線做出來的 pipeline;新能力開里程碑');
    }
  }
  ...
  for (const x of a.info.values()) {
    ...
    const t = p.template;
    if (t.stages || t.laws || t.examples) warn(p.fullName, `還是模板(${[t.stages && `Stages ${t.stages} 列`, t.laws && `Laws ${t.laws} 條`, t.examples && `Examples ${t.examples} 列`].filter(Boolean).join('、')}是佔位符)`, 'lawful:law-design 談完寫成真的');
...
export function suggestRoutes(design, a, ov, warnCount, building = new Set(), inv = invariantView(design, a)) {
  ...
  for (const x of drafts) steps.push(`lawful:law-design ${x.p.fullName}(還是 draft:Law 談完、開發者拍板才改 ready,之後自動接上 build)`);
  ...
  for (const q of ov.reqs) if (q.built && q.accept && q.accept.formal && !q.tested) steps.push(`lawful:build ${q.id}(建置路線達成,只派 qa 寫 ${q.id}#ACCEPT 的驗收測試)`);
  ...
  if (!steps.length) {
    ...
    if (!a.info.size) note = `還沒有任何 pipeline;${ov.objs.length ? 'lawful:objective 給第一條里程碑一個英文名,再 lawful:spike-impl <M-n-slug> 做第一條切片' : '先 lawful:objective 訂目標與里程碑,再 lawful:spike-impl <M-n-slug>'}`;
    else if (allDone && lawsFalse.length) note = `每條 pipeline 達成,但${lawsFalse.join('、')}:先補上,不加新功能`;
    else if (allDone && !warnCount) note = '目前功能全部正常運作:每條 pipeline 達成、每條需求達成、每條領域不變量成立、測試全綠、沒有 open GAP、沒有警訊。沒有非做不可的事,可以加新功能:lawful requirement add 或 lawful:objective';
...
export function statusReport(design, source, adapter, results, resultNote, building = new Set(), stale = new Set(), phases = new Map()) {
  ...
  out.push('');
  out.push('## 需求');
  if (!cone) out.push(`- 沒有 Cone.md;${design.legacySystem ? 'lawful migrate cone --write' : 'lawful:project 建它'}`);
  else if (!ov.reqs.length) out.push('- 沒有任何需求;lawful requirement add <一句話> --accept <句>');
  else {
  ...
  if (!ov.objs.length) out.push('- 沒有任何目標;lawful:objective 訂第一個');
    ...
    for (const o of ov.objs) {
      ...
      if (next) out.push(`- ${o.id} 下一個里程碑:${next.id} ${next.title}${next.docs.length ? `(${next.docs.map((d) => `${d.name} ${state(d)}`).join('、')})` : (next.slug ? `(還沒有切片:lawful:spike-impl ${next.fullName})` : '(還沒有切片,也還沒有英文名)')}`);
      else {
  ...
  for (const x of a.info.values()) if (x.p.status === 'draft') {
    deciding++;
    out.push(`- ${x.p.fullName} 還是 draft:lawful:law-design 談完、開發者拍板才改 ready`);
  }
