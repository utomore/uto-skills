...
**`.lawful/` 之外的一個檔**:專案根目錄的 `CLAUDE.md`。它是開發者自己的檔,每一場 session 都會載入;這套流程只讀寫它的「## 名詞」節,**這一節以外的內容一個字都不動**。這一節是一張表(`templates/glossary.md`):
...
- **名詞只住這一節**:新增或調整任何一個領域名詞的定義,一律改專案根目錄 `CLAUDE.md` 的「## 名詞」節,不寫在別處——不寫進 `.lawful/` 的任何一份文檔、不寫進 ADR、不在各條 pipeline 的 Brief 裡另外定義一次。需求的一句話、驗收、里程碑那一句、law 的第一句裡用到的領域名詞,都要在這張表上,寫法照表上的。
- **三欄**:名詞(需求與 law 裡怎麼寫就怎麼寫)、定義(一句話:它是什麼、不是什麼)、型別(程式碼裡對應的型別名,還沒有就寫 `-`)。型別欄填了而程式碼裡找不到,`lawful lint laws` 紅。
- **誰寫**:
  - `lawful:kickoff` 建立這一節:專案根目錄沒有 `CLAUDE.md` 就建一份,只放 `# <專案名>` 與這一節;已經有就在檔尾補這一節。把願景裡已經出現的領域名詞與開發者逐個講定寫進去,只有一兩個也行。
  - `lawful:require-design`:需求的一句話、驗收或里程碑那一句裡出現表上沒有的領域名詞,**先**與開發者講定它的定義、寫進表,**才**寫需求檔。開發者要改一個既有名詞的定義也走它:先攤影響範圍(哪幾條需求的句子、哪幾條 pipeline 的 law 用到這個詞;laws.md「影響範圍與選項」),給「改 / 不改」,開發者明確說要才改;因此要調整的 law 各自走 `lawful:scope-laws`。
  - `lawful:scope-laws`:這一片讓某個名詞第一次有了型別,把型別名填進那一列的「型別」欄;它只動這一欄。表上的型別在一次修訂裡改了名,做那次修訂的 skill(`scope-revise` 或 `scope-laws`)把這一欄的名字跟著換,是機械同步,定義不動。
  - 其餘的 skill 不寫這一節。`lawful:spike-impl` 取型別名、函式名時照表上的名詞。
- 一個名詞的意思要改、牽動很多文檔而且回不了頭,那是一個決定:名詞表照改,理由由 `lawful:integrate` 寫成 ADR(「ADR」)。
...
| 需求 | `requirements/R-n-<slug>.md`,一條需求一個檔;frontmatter `id`、`priority`、`updated`,標題 `# <全名>:<一句話>` | 一件使用者要得到的事,**必須達成**:一句話講誰在什麼情況下要得到什麼。各有一句**驗收**(`- 驗收:…`):可驗證、可判定的一句話,開發者審核這條需求時照它判;最好有一條歸屬 `R-n#ACCEPT` 的驗收測試(roles.md「驗收測試」)。另有一張**驗收記錄**表(`## 驗收記錄`,日期 / 誰 / 憑據 / 結論),裝開發者親自審核的結果,只由 `lawful requirement accept` 寫(「需求的達成只有人判得了」)。各有一級**優先**(frontmatter `priority`,1 到 4,1 最高)。需求不是 law:做到之前它本來就還沒達成。需求只在與開發者的討論裡成形,只由 `lawful:require-design` 寫。`R-n` 是流水號,永不重排,只是身分,不代表先後;slug 是 kebab-case 英文,講這條需求要得到什麼。至少一條 |
| 優先各級 | `Cone.md`「Constraint」一行 `- 優先:1 = …;2 = …;3 = …;4 = …` | 各級在這個專案代表什麼,專案自己定(例:1 = 主軸與它的直接前提;2 = 地基;3 = 呈現與存取;4 = 工具與詞彙);`lawful status` 印它,沒宣告列警訊 |
| 里程碑 | 需求檔裡的表 `里程碑 \| 做到什麼 \| 綁定 \| 怎麼驗` | 這條需求的建置切成的階段,**有順序,依序完成;全部達成,這條需求的建置就走完**。第一格是全名 `M-n-<slug>`:`M-n` 是配號用的編號,全資料夾唯一(跨需求檔);slug 是 kebab-case 英文,切片的分支 `build/M-n-<slug>` 與決策紀錄以全名為鍵;表的列序就是先後。**綁定**欄是 pipeline 全名,「、」分隔,切片做出來的新 pipeline 或這個階段靠修訂做到的既有 pipeline 都可以;「-」的意思是**還沒有切片**。**怎麼驗**欄是一道跑起來看得到這一句話的指令:開發者審核這條需求時照它跑,`lawful status` 把它印成步驟;切片收尾時由 `lawful:integrate` 從決策紀錄的「Entry」搬過來(roles.md「決策紀錄」),談需求的當場就知道怎麼驗的直接寫(`lawful requirement milestone … --verify`),還沒有的寫「-」 |
...
- **領域名詞先在名詞表上**:需求的一句話、驗收、里程碑那一句裡用到的領域名詞,要在專案根目錄 `CLAUDE.md` 的「## 名詞」節上,寫法照表上的(「`.lawful/`」)。表上沒有的,`lawful:require-design` 先與開發者講定它的定義、寫進表,才寫需求檔;名詞的定義不寫在需求檔裡。
...
- **靠修訂達成的里程碑**:一條里程碑不一定做出新的 pipeline。把既有的東西改快、改小、改好(效能、大小、訊息、演算法),或這個階段是靠修訂既有的 pipeline 做到的,就是這條需求底下再加一條里程碑、綁既有的 pipeline:`lawful:require-design` 談需求的當場就切出來、當場綁(`lawful requirement milestone <R-n> <slug> "<一句話>" --bind <pipeline 全名>`)。切的判準不變:那一句是使用者看得到的那一句(寫「存出來的檔案不超過 1 MB」,不寫「存檔太大」),展示的方法是量得到的那道指令。
...
- 需求的驗收可以寫成同一套三行式,只差識別字可以是任何一條 pipeline 的 Stages 簽名(不含 `!` 列);寫了三行就承諾了驗收測試(roles.md「驗收測試」),沒有測試時證據不齊;寫不成三行就只留一句話,證據由里程碑全部達成承接。`lint laws` 對帳。領域不變量的三行式照 laws.md「全域 Law」,`lint invariants` 對帳。
...
**需求的驗收**不是 law,也不掛在 stage 上,掛在「這件事達成了沒」上:一條需求一句。它講的是使用者看得到的結果,不是某個函數的性質;能寫成三行、有驗收測試最好(歸屬 `R-n#ACCEPT`,誰寫、什麼時候寫見 roles.md「驗收測試」),不能就一句可判定的話。兩種都只是**證據**:達成與否由開發者親自審核(「需求的達成只有人判得了」)。
...
誰做,一句話分流:**要調整(修改、放寬、替換、刪除)既有的 law → `lawful:scope-laws`;law 不動、或只新增 law,而文檔或實作要變 → `lawful:scope-revise`;全域 Law → `lawful:global-laws`;需求面的條目(需求、驗收、優先、里程碑)→ `lawful:require-design`。** **一件修訂從頭到尾只有一個修訂類的 skill(`lawful:scope-laws` 或 `lawful:scope-revise`)在跑,跑到 `verified` 為止**:不交錯、不接力,一條 REV 裝下這一件的全部。

| | `lawful:scope-laws` | `lawful:scope-revise` |
|---|---|---|
| 收什麼 | 任何一條既有的 law 要修改、放寬、替換或刪除,連同這一件修訂連帶要改的 Stages 簽名、型別、模組、Examples、新增的 law 與實作方向,一手包辦;刪除 stage(連帶刪它的 law),含兩條 pipeline 重複的 stage 留一份、另一條改成引用;文檔退役;既有 example 的輸入輸出要變;還沒 `verified` 的 pipeline(切片那一波)的任何改動 | `verified` 的 pipeline,既有的 law 一條都不動:Stages 的簽名或型別要改、新增 stage、stage 跨層搬家、層的歸屬修正、`=` 列搬到別的模組單元而要換 slug、Brief / 決定 / 描述要改、靠修訂這條 pipeline 達成的里程碑(效能、大小、訊息、演算法這類實作品質,或新的承諾)、law 在而實作不符或行為沒有 law 守著的 bug、答案不必調整既有 law 的 GAP;過程中可以**新增** law(保護用的、效能的新上界、新 stage 的)與新的 example |
...
- 依:來源與那一句話(GAP 的提問原句、ADR 全名、靠修訂這條 pipeline 達成的里程碑全名 `M-n-<slug>` 與它那一句、開發者的話、整合的仲裁選了什麼、整合時留了哪一份重複的 stage)。靠修訂達成的里程碑一律從這裡進來:它綁的每條既有 pipeline 各一條 REV,依欄寫里程碑的全名,`lawful status` 靠它判這條里程碑達成了沒(「完成度」);這種 REV 的保護一定含需求的驗收引用到的每條 law,修訂不准讓需求退回未達成。
...
- 驗收記錄**只由 `lawful requirement accept` 寫**,開發者不手寫、skill 也不代簽(roles.md「誰動哪個檔」)。憑據欄寫的是跑了哪道指令、看到什麼,不是「看起來沒問題」。
...
| 東西 | 怎麼判 |
|---|---|
| 里程碑達成 | 綁定的每條 pipeline 都達成;靠修訂達成的里程碑(它綁的 pipeline 先被排在前面的里程碑綁過,「願景、需求與里程碑」)另外要那條 pipeline 的修訂記錄裡有一條依欄引用它的 REV。還沒有那條 REV 是「待修訂」;有 REV 而 pipeline 未達成是「進行中」 |
| 需求的建置進度 | 達成的里程碑 / 里程碑數;全部達成,這條需求的建置就走完。沒有里程碑的需求印「-」並列警訊 |
| 全域 Law | 三類各一列:那一道 lint 幾條不合規;契約欄指到的 law 幾條成立;領域不變量幾條成立(laws.md「全域 Law」) |
| 需求的證據 | 機器算得出來的那一半:有歸屬 `R-n#ACCEPT` 的驗收測試就以它為準,測試綠 = 證據齊;沒有驗收測試時,里程碑全部達成 = 證據齊。驗收寫了三行式卻沒有測試、測試在而沒跑,都是「未知」,證據不齊;驗收還是模板、一條里程碑都沒有是「未達成」。里程碑全部達成而驗收測試沒過是警訊:里程碑切漏了,或驗收寫錯,回 `lawful:require-design` |
| 需求達成 | **只由開發者親自判,`lawful status` 不自己算**(「需求的達成只有人判得了」)。證據齊而還沒有人審是「待審核」;開發者審過、`lawful requirement accept` 寫進驗收記錄是「已驗收」;簽過之後證據翻掉是「待重審」;證據還沒齊是「建構中」 |
