...
1. 今天能開幾條線:`ready`、沒 open GAP、引用的子流全部達成的 pipeline;每條附它綁在哪個目標與里程碑,照目標優先排。專案根目錄有 `.git` 時查分支:有 `build/<全名>` 分支而且它還沒合進主線的列成建構中,不算能開;已經合進主線的分支是殘留,不擋任何線。兩條能開的線的 stage 住同一個模組,附一行提示:同時開,整合時那個模組的檔會兩邊都動
