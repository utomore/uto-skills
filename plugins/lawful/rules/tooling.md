...
| `migrate cone [--write]` | 只有 `system.md` 的樹,換成 `Cone.md` 體系:先印帳本,`--write` 才落地。願景與目的併成「願景」、「語言與工具」節的內容進「Constraint」、邊界與對外 I/O 收進「全域 Law」區、Pipelines 表的類別寫進各 pipeline 的 `kind`、里程碑補英文名、優先各級那行搬進「Constraint」,最後刪 `system.md`;需求面的條目接著跑 `migrate requirements` 收進 `requirements/` |
...
- **需求**:先一行「Constraint」宣告的優先各級,再每條需求一列(需求、優先、一句話、**審核**:已驗收 / 待審核 / 待重審 / 建構中 / 未知 / 未達成與判定來源、**依賴**、里程碑總數、里程碑達成、完成度),照優先、需求編號排。「依賴」欄從 pipeline 的引用推,沒有依賴印「-」;表後每條沒達成的需求一行「下一條里程碑」,附綁定的 pipeline 各在什麼狀態,還沒綁的寫成還沒有切片與開它的命令,靠修訂達成而還沒有 REV 引用它的寫成待修訂與修訂它的命令;最後一行列沒有被任何里程碑綁定的 pipeline。**這一段答的是「必須達成的事達成了沒」**
