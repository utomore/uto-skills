...
2. **分類每條紅**:簽名不一致看兩邊誰對,文檔錯走 `lawful:revise`、程式碼錯列給 impl;同層搬家直接 `lawful sync`;未登記模組 `lawful modules --gen` 再請開發者填層;沒匯出的 stage、沒有匯出清單的模組列給 impl;跨層 import、pure 碰效果型別、production import `*.Internal`、`=` 列在 shell、對外 I/O 對不上 IO 介面,列為結構問題。
