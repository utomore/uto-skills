...
從 `status` 的需求表、表後每條需求的「下一條里程碑」、全域 Law 表讀，里程碑的原文開需求檔看，九題：
...
4. **誰不朝向任何需求**：沒被任何里程碑綁定的 pipeline、沒有里程碑的需求、沒有優先的需求、綁到不存在的 pipeline 的里程碑（`status` 的警訊）。還沒有切片的里程碑與待修訂的里程碑列出來但不是問題。
