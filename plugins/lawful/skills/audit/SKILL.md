...
`<L>` 解析一次(`rules/tooling.md`「CLI」)。一次讀完:`rules/tooling.md`「CLI」、`rules/boundary.md` 全份、`rules/pipelines.md`「節」「完成度」。
...
3. **人判 laws**:每條 pipeline 拿種類問法表(`pipelines.md`「節」的 Laws 表)逐種對:該有 invariant 的有沒有、roundtrip 有沒有說哪些欄位不算、bound 有沒有數字。缺的寫成提議,不直接加。
