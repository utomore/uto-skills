---
name: design
description: lawful 的系統設計 — 訪談後產出 .design/system.md(目的、語言與工具、四層邊界、對外 I/O、里程碑 pipeline 清單)與 modules.md 模組表,跨 pipeline 的決定寫 ADR,每條里程碑用 lawful claim 建成 draft。觸發詞:系統設計、開新專案、lawful design、建 .design、模組表、邊界、里程碑。Use when starting a pure-functional project or reshaping its layers and milestones.
user-invocable: true
---
...
`<L>` 解析一次(`rules/tooling.md`「CLI」)。一次讀完:`rules/pipelines.md`「`.design/`」「system.md」「ADR」、`rules/boundary.md` 全份、`rules/tooling.md`「language adapter」「收尾定錨」。

## 輸入 / 產出

| 輸入 | 產出 |
|---|---|
| 開發者的意圖、既有程式碼(有的話) | `.design/system.md`、`.design/modules.md`、需要的 `adr/`、每條里程碑一份 `draft` pipeline |

## 步驟

1. **看現況。** 有 `.design/` 就是更新模式:只改開發者點名的節,其餘不動。沒有就建。
