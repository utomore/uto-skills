...
3. 組好內容**直接 `gh pr create` 送出,不需先向開發者確認**(發完在 §6 回報大綱):
   ...
   - **內文**:繁體中文,章節固定:
     ...
     | 全名 | 類別 | 分支 | 需求 · 里程碑 | 達成 |
     |---|---|---|---|---|
     (類別欄寫 `io` 或 `subflow`;「需求 · 里程碑」欄寫需求編號與里程碑全名,如 `R-1 · M-1-save-write`。立案分支:全名欄寫改了哪條需求、里程碑、調整、模組單元或全域 Law,達成欄寫「-」;對不到 pipeline 的分支:全名欄寫分支名,其餘欄寫「-」)
     ...
     - 這次 PR 帶進來的變更:<INV-n / 四層 / 對外 I/O:改了什麼、開發者哪一句話批准的、ADR-00x>;只來自 `plan/` 分支上 global-laws 落筆的;無則「無」
