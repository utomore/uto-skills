...
| 輸入 | 產出 |
|---|---|
| 開發者要得到什麼、既有的需求與 pipeline、專案根目錄 `CLAUDE.md` 的名詞表 | `Cone.md`「Constraint」一行優先各級的宣告;`CLAUDE.md`「## 名詞」節裡新講定的名詞與改過的定義;以及 `requirements/` 裡一條需求一個檔 `R-n-<slug>.md`:一句話、驗收、優先、照先後排好的里程碑(全名 `M-n-<slug>`,綁定欄填它做出來的 pipeline、或它靠修訂達成的既有 pipeline,還沒有切片的留「-」) |
...
1. **優先怎麼分**:`Cone.md`「Constraint」要有一行「- 優先:1 = …;2 = …;3 = …;4 = …」宣告各級在這個專案代表什麼(例:1 = 主軸與它的直接前提;2 = 地基;3 = 呈現與存取;4 = 工具與詞彙)。沒有這一行、或還是模板就先跟開發者訂,寫進那一節;有了就照它問。
