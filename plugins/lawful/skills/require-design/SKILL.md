...
上面這幾段（一份輸出切成幾段，每段一道指令；沒有內容的那幾道是空的）是載入 skill 時跑 `lawful brief require-design` 的輸出：規章、`Cone.md` 全文（願景在裡面）、`requirements/` 底下每個需求檔全文、`lawful status` 的需求表、全域 Law、等決定、警訊與建議路線。開工要讀的規章與專案現況都在這裡，不再另外讀。名詞表不在 brief 裡：它住專案根目錄 `CLAUDE.md` 的「## 名詞」節，`CLAUDE.md` 每一場 session 都已經載入。
...
| 輸入 | 產出 |
|---|---|
| 開發者要得到什麼、既有的需求與 pipeline、專案根目錄 `CLAUDE.md` 的名詞表 | `CLAUDE.md`「## 名詞」節裡新講定的名詞與改過的定義；以及 `requirements/` 裡一條需求一個檔 `R-n-<slug>.md`：一句話、驗收、優先、照先後排好的里程碑（全名 `M-n-<slug>`，綁定欄填它做出來的 pipeline、或它靠修訂達成的既有 pipeline，還沒有切片的留「-」） |
