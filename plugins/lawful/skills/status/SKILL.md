...
3. **講人話**:不重印報告;先講願景與目標,再照七段講「今天該做什麼、為什麼」。每段一到三句,pipeline 寫全名;警訊那張表照抄怎麼辦欄。第 1 段的線可以同時各開一波(`roles.md`「分支與所有權」),建構中的講它在哪條分支等整合;報告附了共用單元提示就照講。
...
5. **看板**:看板每次都產,不必等開發者開口。它是一棵從願景往下長到便利貼的樹:一層優先度一個區塊,區塊底下目標一層、里程碑一層;引用畫成虛線箭頭,箭頭那一端是被引用的那一份。點便利貼,右邊那一欄換成那一份的細節。側欄首頁另有一段模組:一個單元一列、層是它在原始碼樹裡的落點,列裡的 pipeline 點得進去。開發者要直接跳出瀏覽器就加 `--open`。看板不取代第 3 步的講人話。
