...
3. **講人話**:不重印報告;先講需求、全域 Law 與模組,再照七段講「今天該做什麼、為什麼」。每段一到三句,pipeline 與里程碑都寫全名(`P-001-save-write`、`M-1-save-write`);警訊那張表照抄怎麼辦欄。第 1 段的線可以同時各開一波(`roles.md`「分支與所有權」):還沒有切片的里程碑是 `lawful:spike-impl`,`ready` 的 pipeline 是 `lawful:build`;建構中的講它在哪條分支、走到哪一步、下一個動作是誰的(切片完成等 `lawful:scope-laws`、Law 討論中等開發者拍板與全域的候選、達成等 `lawful:integrate`);報告附了共用單元提示就照講。要改既有 pipeline 的線照一句話分流講:要調整(修改、放寬、替換、刪除)既有的 law 是 `lawful:scope-laws`;law 不動、或只新增 law,而文檔或實作要變(含待修訂的里程碑:靠修訂達成、還沒有 REV 引用它)是 `lawful:scope-revise`;全域 Law 是 `lawful:global-laws`;需求面的條目是 `lawful:require-design`。open 的 GAP 照同一句講回答該走哪一個。
   - **需求**(從報告的需求表與表後每條需求的「下一條里程碑」那一行講):每條需求達成、未達成還是未知,判定來源是驗收測試還是由里程碑全部達成推得;里程碑全部達成而驗收沒過的(里程碑切漏了,或驗收寫錯,回 `lawful:require-design`),明講「驗收沒過」;驗收寫了三行卻沒有驗收測試的,講建議路線裡那條 `lawful:build R-n`。需求表的「依賴」欄講哪條需求疊在哪條上面(從 pipeline 的引用推的,不是人寫的):互不依賴的可以同時開工,有依賴的,被依賴的那條沒達成,依賴它的就還不算達成。最高優先還沒達成的需求是哪一條、里程碑達成幾條、完成度幾 %、它的下一條里程碑卡在哪條 pipeline、還沒有切片、還是待修訂(靠修訂達成、還沒有 REV 引用它:講它綁哪一條、走哪一個修訂的 skill);有沒有 pipeline 沒被任何里程碑綁定、有沒有需求沒有里程碑;正在做的事是不是最高優先需求的下一條里程碑,不是就明講;願景還是模板就先講這件事。答的是「必須達成的事達成了沒、我們有沒有朝向需求」。
   - **全域 Law**(從那兩張表講):三類各自那一道 lint 有沒有紅(架構 `lint boundary`、契約 `lint io`、領域不變量 `lint invariants`),有紅先講這件事:不得違反的約束被踩到了;契約欄指到的 law 幾條成立。領域不變量哪一條未成立(有程式碼違反了全專案的規則,先講這件事)、哪幾條還未知(還沒有三行式,或寫了三行卻沒有測試,講建議路線裡那條 `lawful:build INV-n`)。一條都沒有就一句帶過。三個小區都是空的(沒有領域不變量、四層「裝什麼」那幾句還沒寫、對外 I/O 0 列)是正常的:全域 Law 是從切片裡抽上去的(`laws.md`「全域 Law」),講一句「全域 Law 還沒有,等第一條切片談完約束抽出來」,不當成問題;第 1 段印了「專案的第一條切片單獨走完:build/<鍵> …」那一句就照講:那條分支抽出全域 Law 並合進主線之前,不開別的切片。
   - **模組**(從模組表講):這批工作落在哪幾個單元、有沒有單元還沒有任何 stage 住進去、有沒有宣告了層卻還沒有程式碼、有沒有程式碼在模組表外面。答的是「東西住在哪」,不是進度。
   - 第 7 段(建議路線)是從最高優先的需求第一條沒達成的里程碑推出來的,還在等別條需求的排後面;回答 GAP 與待修訂的里程碑各走哪一個 skill,照上面那一句分流講;它寫「目前功能全部正常運作,可以加新功能」時,下一步是 `lawful:require-design`(新需求,或既有需求底下的新里程碑)。
