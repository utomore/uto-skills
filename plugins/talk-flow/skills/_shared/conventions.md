...
共用詞彙表:`styles.md`(風格基底)、`layouts.md`(版型)、`layers.md`(前景/背景分層)、`diagrams.md`(圖形)、`wording.md`(**文案語感**:禁用句型與人稱指稱,產文與審查都以它為準)。
...
|---|---|---|---|
| Level 1 主軸 | `/topic-design` | 核心訊息、聽眾、時長、段落切分、風格基底、**視覺規範的語意**(強調有哪幾種各代表什麼、背景幾套各對應演講的什麼結構) | 寫到「語意與數量」為止。**禁止**寫入任何 CSS 數值(px、色碼、opacity)、class 名或槽位編號 — 那些屬於 theme.css 與 Level 3 |
| Level 2 段落 | `/section-design` | 每頁要說服聽眾什麼、內文放什麼、要不要圖、圖回答什麼問題、背景的**語意需求**(這段要不要跟別段區隔) | 寫到「這頁講什麼」為止。**禁止**指定版型類別、`bg-*` 類別、圖形畫法與座標 |
| Level 3 實作 | `/section-impl`、`/page-adjust` | 版型、視覺動線、文字技法的逐頁選用、SVG 幾何、背景類別、theme.css tokens 的實際數值 | 在 Level 1 語意規範與 Level 2 內容規劃之內,**擁有完全的呈現自主權** |
...
- **上層不因下層的內部選擇而改動**:Level 3 換個版型、調個座標、微調 opacity,不需要也不該回頭改 `topic.md`;只有「語意或數量本身要變」才回上層
...
- **文案語感**:投影片上的每一句都直接講事情。**禁止對比翻轉句**(「A,不等於 B」「是 A,不是 B」「不是 A,而是 B」「不只是 A,更是 B」…同一句裡否定再翻轉),**禁止人稱代名詞**(我/我們/你/你們/大家/各位),禁止空泛詞與無資訊標題;要對比就交給版面(兩欄、表格、✓/✗、前後兩頁)。完整禁區、爛範例對照與自檢清單見 `_shared/wording.md`,產文與審查都以它為準
...
- **備註**:每頁結尾一個 `<!-- ... -->` 註解作 presenter note,**只寫提醒**(要點、關鍵措辭、時間提醒),1–5 行,禁止逐字稿、情境描述與口播人稱(「我要跟大家說…」);段落第一頁備註以 `銜接:` 開頭(怎麼接上一段)、最後一頁含 `交棒:`(怎麼帶到下一段)
