...
- **架構圖的幾何靠量測不靠目視**:標籤有沒有溢出、連線端點有沒有貼齊、線有沒有穿過節點、尺寸間距是否一致,用 `/svg-layout`(normalize → inspect → lint)取得數字;不要直接讀 SVG 原始碼猜座標
...
- 配色、字體、背景只改 `theme.css` 的 tokens(CSS variables),不在單頁內寫死色值或背景路徑;全簡報一致(真正一次性的單頁背景才用 `<style scoped>`)
