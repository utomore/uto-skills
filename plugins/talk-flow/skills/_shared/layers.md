...
- `/topic-design`(L1)定**語意與數量** — 要不要背景、幾套、各套對應演講的什麼結構(封面/轉場/某段/某類頁),寫進 `topic.md`「投影片分層」章節;同時把起始值填進 `theme.css` 的分層 tokens
- `/section-impl`(L3)逐頁選背景類別,並在**第一段實作完成後做強度驗收**(那時才有真實頁面可看),需要時直接調 tokens 數值 —— 數值調整不必回頭改 `topic.md`,語意或套數要變才回去
...
1. **這場需要背景嗎?**(L1)資訊密度高、印表機友善、正式匯報 → `none` 就是答案,不要為了豐富而豐富
2. **素材從哪來?**(L1)使用者有品牌背景圖/照片 → 資產檔;沒有 → 跟使用者討論風格關鍵字(柔和有機 / 幾何弧線 / 格線科技 / 單側色帶),用漸層或現畫 SVG
3. **要幾套、各代表什麼?**(L1)先問全場一套夠不夠;要多套就寫明「哪一組頁面用哪一套、為什麼」記進 topic.md,並填進 `--bg-image-2` / `--bg-image-3`
4. **強度多少?**(L3)tokens 先給偏淡的起始值(0.35–0.5),等第一段頁面實作出來,在最文字密的那頁看了再定
5. **哪些頁要關?**(L3)圖表頁、截圖頁、密集表格頁一律 `bg-none`;封面與 divider 通常 `bg-strong`
6. **驗收**(L3)第一段實作完成後 build:看最密的一頁(文字讀得清?)與最空的一頁(畫面不空?)、再把用到的每一套背景各看一頁 —— 全過才算定案;調整只動 theme.css 數值,不回頭改 topic.md
