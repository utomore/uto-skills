...
先讀取 `../_shared/conventions.md`、`../_shared/layouts.md`、`../_shared/layers.md`、`../_shared/diagrams.md` 與 `../_shared/wording.md`,遵守所有文檔慣例、版型詞彙、分層詞彙、圖形詞彙與**文案語感規範**(改到任何一個字都受它約束)。
...
- **內文**:文字內容、措辭、內文形式(條列/表格/段落)互換;改寫出來的每一句都要過 `wording.md` 的自檢 —— **不得出現對比翻轉句**(「A,不等於 B」「是 A,不是 B」「不只是 A,更是 B」…)與**人稱代名詞**(我/我們/你/你們/大家/各位),使用者想強調對比就改用版面承載(兩欄、表格、✓/✗、拆成兩頁);使用者自己指定的措辭照做,並記進該頁備註或 `topic.md`「文案語感例外」。字級、強調、列表符號仍只從 `topic.md`「文字規範」選用 — 使用者要的效果不在規範內時,先確認是「改規範」(回 `/topic-design`,全簡報生效)還是換一種規範內的作法
...
- `node build.mjs`(含 `outputs` 的其他格式)重新建置,在瀏覽器開 `talk/dist/slides.html` 跳到該頁(或請使用者開)確認:文字不溢出、grid 沒塌陷、圖形清晰、背景沒搶走第一眼且正文讀得清、與前後頁風格銜接自然;**這頁的每一句(含圖形標籤與備註)跑一次 `wording.md` 的自檢清單**
