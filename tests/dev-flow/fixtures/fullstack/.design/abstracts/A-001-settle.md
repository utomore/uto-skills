...
| # | 簽名 | 做什麼 | 模組 | 層 |
|---|---|---|---|---|
| 1 | `sumLines(MoneyList): Settlement` | 同幣別的品項相加,幣別不同就是錯誤 | `web/src/domain/money.ts` | domain |
| 2 | `subtract(Settlement, Money): Settlement` | 扣掉一筆金額,扣成負的就是錯誤 | `web/src/domain/money.ts` | domain |
...
| = | `settle(MoneyList, Money): Settlement` | 整條:加總再扣折扣 | `web/src/domain/money.ts` | domain |
...
- **重開一次**(2026-09-07):建檔即被 REV-1 記錄,收整做完由 build 重新 verified。

## 修訂記錄
- REV-1(2026-09-07,依 dev-flow:refactor 收整):把 F-001-checkout 與 F-002-refund 各自寫的金額計算收整進本檔
  - 動到:sumLines、subtract、settle 三條簽名搬進 `web/src/domain/money.ts`
