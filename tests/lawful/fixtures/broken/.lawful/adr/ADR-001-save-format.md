...
## 情境
P-001-game-save 要把 SaveState 寫成檔案。實體數上千時檔案大小與寫入時間會被玩家感覺到;格式一旦出貨就要永久讀得回來。
...
## 後果
- `Game.Save.Codec` 依賴 `cborg`,住 core 層,不碰 IO。
- 之後加版本欄要在 P-001-game-save 走 REV,動到 `decode` 的簽名。
